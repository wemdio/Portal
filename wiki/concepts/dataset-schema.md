...
## ⚠️ Временной горизонт — НЕ «годы», два слоя (2026-05-31)

Датасет содержит **НЕ несколько лет** детальных данных (распространённое заблуждение, исходно записанное при пуле — неверно). Реально:

| Слой | Горизонт | Чем владеем |
|---|---|---|
| **Гранулярный** (`raw_emails`: каждое письмо, по дням, тексты/темы/треды) | **отправки: 11 дек 2025 → 29 мая 2026** (~5.5 мес); ответы: с авг 2025 | детальный анализ |
| **Агрегатный** (`raw_campaign_analytics_overview_snap`: lifetime-итоги) | **с начала 2025** (581 старая кампания, ~1.2M отправок) | грубо «итоги кампании/клиента за жизнь» |

**Почему нет детали до дек 2025:** Instantly хранит письма (особенно отправленные) ~5-6 месяцев, потом **стирает**. Проверено: живой `/emails` по кампании от фев-2025 отдаёт **0 писем**. Старые кампании сохраняют метаданные + lifetime-агрегаты, но per-email история утеряна **безвозвратно** (≈1.2M старых отправок — только в агрегате). Расширить детальную историю назад нельзя — данных нет у источника.

**Вперёд:** nightly sync пишет всё → гранулярная история полна с дек-2025 и далее.

**Практика:** любой временной анализ ответности/тем держи в окне **дек 2025 – май 2026** (статистически плотно — с фев 2026). Для старых кампаний — только overview-агрегаты, не per-email.
