...
`raw_emails` сейчас выглядит как «отправки с 11 дек 2025» — это **артефакт оборванного первого пула**, НЕ предел Instantly. Причина: `/emails` отдаёт новейшими-вперёд, а пул рвался на 429/сети → успевал свежие страницы, не доходил до старых. **Проверено живым API:** кампания «ProdavAI» (создана авг-2025, lifetime 16916, в датасете 0) — live `/emails` отдаёт 471+ писем от **авг 2025**. Данные за 2025 ЖИВЫ.

**Можно дотянуть до ~фев 2025** (старт студии) = ~15 месяцев гранулярных данных. Нужен backfill-пул недотянутых кампаний (на безопасных ~10 RPM, как финальный пул — несколько дней, инфра есть: `pull.mjs --phase=emails`, per-campaign кэш докачивает пропущенное).

⚠️ **До backfill любой временной анализ смещён** — кажется что история начинается в декабре, а это просто что выкачалось. Для масштаба используй `overview` lifetime, не `count(raw_emails)`.

> Ранее я ошибочно записал «Instantly стёр старое, расширить нельзя» — НЕВЕРНО (тестировал «пробную» кампанию, которая никогда не слала → ложный 0). Данные доступны.
