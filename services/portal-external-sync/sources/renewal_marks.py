"""Автоподтверждение продлений по комментарию AMO, тексту задачи AMO и по
project_type — в порядке приоритета.

Целиком в SQL-функции apply_renewal_marks() (см.
supabase/migrations/20260803_0002_renewal_marks.sql, переопределена в
supabase/migrations/20260803_0004_renewal_marks_note_text.sql добавлением
приоритетного сигнала по комментариям) — тот же принцип, что у
...
Место в SOURCES (main.py) — СТРОГО после AmoNotesSync(), AmoTasksSync() и
после банковских источников (BankTochkaSync/BankTBankSync): функция ищет
комментарии вида «Продление N - сумма» в свежих amo_notes, задачи со словом
«продл»/«пролонг» в свежих amo_tasks, и сверяет то и другое с только что
приехавшими платежами bank_transactions — всем трём нужны данные ЭТОЙ же
ночи, иначе подтверждение отстаёт на сутки от того, что могло бы
подтвердиться сразу.
"""
