"""Тесты на watermark и защиту от усечения в AmoTasksSync.run().

Отдельно от test_amo_tasks_mapping.py (там — только чистая _to_row).
Здесь проверяется (по образцу test_amo_events_sync.py):
- _watermark читает исключительно amo_tasks, никогда external_sync_runs;
...
- run() не бросает исключение на честном завершении (последняя страница
  неполная);
- верхняя граница окна (filter[updated_at][to]) одна и та же на всех
...
import sources.amo_tasks as amo_tasks_module
from sources.amo_tasks import ENTITY_TYPE, OVERLAP_DAYS, AmoTasksSync
...
class _FakeConn:
    """Заглушка asyncpg.Connection: fetchrow всегда отвечает про amo_tasks,
...
def _task(i: int) -> dict:
    return {
        ...
        "task_type_id": 1,
        "text": "",
        "result": {"text": f"комментарий {i}"},
        "complete_till": 1700000000 + i,
        "account_id": 1,
    }


class _AlwaysFullPageClient:
    """Каждая страница ровно PAGE_LIMIT задач — имитирует хвост данных,
    ...
    async def get(self, url):
        page_limit = amo_tasks_module.PAGE_LIMIT
        tasks = [_task(i) for i in range(page_limit)]
        return _FakeResponse(200, {"_embedded": {"tasks": tasks}})
...
class _OnePartialPageClient:
    ...
    async def get(self, url):
        tasks = [_task(i) for i in range(2)]  # < PAGE_LIMIT
        return _FakeResponse(200, {"_embedded": {"tasks": tasks}})
...
def _patch_run_env(monkeypatch, max_pages: int, page_limit: int) -> None:
    monkeypatch.setattr(amo_tasks_module, "MAX_PAGES", max_pages)
    monkeypatch.setattr(amo_tasks_module, "PAGE_LIMIT", page_limit)
    monkeypatch.setattr(amo_tasks_module, "TOKEN", "fake-token")
    monkeypatch.setattr(amo_tasks_module, "BASE_URL", "https://example.amocrm.ru")
    monkeypatch.setattr(amo_tasks_module, "INTER_PAGE_DELAY_SEC", 0)
...
async def test_watermark_falls_back_to_30_days_when_table_empty():
    ...
    since = await AmoTasksSync()._watermark(conn)

    delta = datetime.now(timezone.utc) - since
    assert timedelta(days=29) < delta < timedelta(days=31)


async def test_watermark_uses_max_updated_at_amo_minus_overlap():
...
async def test_watermark_never_queries_external_sync_runs():
    ...
    amo_tasks) и не больше."""
    conn = _FakeConn(ts=None)
    await AmoTasksSync()._watermark(conn)

    assert len(conn.fetchrow_calls) == 1
    assert "amo_tasks" in conn.fetchrow_calls[0]
...
async def test_run_raises_when_max_pages_cap_is_hit(monkeypatch):
    ...
    monkeypatch.setattr(amo_tasks_module.httpx, "AsyncClient", _AlwaysFullPageClient)

    conn = _FakeConn(ts=None)
    with pytest.raises(RuntimeError, match=r"MAX_PAGES=3"):
        await AmoTasksSync().run(conn)
...
async def test_run_completes_normally_when_last_page_is_partial(monkeypatch):
    ...
    monkeypatch.setattr(amo_tasks_module.httpx, "AsyncClient", _OnePartialPageClient)

    conn = _FakeConn(ts=None)
    total = await AmoTasksSync().run(conn)
...
async def test_run_uses_fixed_upper_bound_across_pages(monkeypatch):
    """filter[updated_at][to] считается один раз до цикла и не должен
    ...
    _patch_run_env(monkeypatch, max_pages=5, page_limit=5)
    monkeypatch.setattr(amo_tasks_module.httpx, "AsyncClient", _RecordingClient)
    ...
    to_values = {
        url.split("filter[updated_at][to]=")[1].split("&")[0] for url in urls
    }
    assert all("filter[updated_at][to]=" in url for url in urls)
    assert len(to_values) == 1  # одна и та же граница на всех страницах


async def test_run_filters_by_entity_type_leads_on_the_api_side(monkeypatch):
    """filter[entity_type][]=leads уходит в каждый запрос — задачи по
    contacts/companies не должны даже скачиваться (см. докстринг модуля)."""
    _RecordingClient.recorded_urls = []
    _patch_run_env(monkeypatch, max_pages=5, page_limit=5)
    monkeypatch.setattr(amo_tasks_module.httpx, "AsyncClient", _RecordingClient)
    ...
    assert all(
        f"filter[entity_type][]={ENTITY_TYPE}" in url
...
def test_name_matches_sync_source_registry():
    ...
    # external_sync_runs.source (supabase/migrations/20260803_0001_amo_tasks.sql)
    # — иначе log_run_start() в main.py уронит весь ночной цикл, а не один
    # источник.
    assert AmoTasksSync.name == "amo_tasks"
