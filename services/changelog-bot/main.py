...
Все последовательные изменения одного параметра/правила схлопывай в ОДИН пункт с итоговым значением. Промежуточные значения и обороты «затем», «позже», «снижен с X до Y» не показывай. Коммиты в списке идут от свежих к старым — при сомнениях итоговым считай значение из самого свежего коммита.
