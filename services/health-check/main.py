...
# 20 минут: baseConstructor-воркер сам подбирает осиротевшую задачу по
# истёкшей аренде — BASE_CONSTRUCTOR_LEASE_SECONDS=300 (5 мин,
# docker-compose.prod.yml), а на чистом редеплое аренда обнуляется сразу и
# задача уезжает за секунды. Пока пороги были равны (15 и 15), каждый деплой,
# заставший работающие задачи, давал ложную тревогу за секунды до самоподбора —
# инцидент 11.08.2026, три задачи конструктора баз. Порог монитора обязан быть
# заметно больше порога автоподбора; 20 против 5 — запас четырёхкратный, и
# снижать его вслед за арендой не нужно: он ловит не потерю исполнителя, а
# задачу, которая стоит на месте с живым исполнителем.
...
_JOB_MONITOR_SPECS: tuple[JobMonitorSpec, ...] = (
    ...
    JobMonitorSpec(
        "website_inn_lookup_jobs", "Поиск ИНН по сайтам", ("preparing", "pending", "running"),
        ("total", "processed", "found"),
        "portal-worker-website-inn-lookup", updated_column="updated_at",
    ),
    JobMonitorSpec(
