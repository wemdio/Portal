...
#   Очередь client_manual_score_runs (ручной скоринг) в списке ниже тоже
#   НИКОГДА не значилась — деплой просто убивал воркер посреди прогона, и
#   прогон оставался в статусе processing навсегда: прежний цикл выбирал только
#   pending и подобрать его было некому. После переезда на общий жизненный цикл
#   это перестало быть потерей: воркер отпускает аренду по SIGTERM, а
#   продолжение идёт по строкам с непроставленным бакетом. Добавлять таблицу
#   сюда не нужно — только сам сервис в список ниже.
...
is_lifecycle_managed_worker() {
  case "$1" in
    ...
    worker-manual-scoring) return 0 ;;
    *) return 1 ;;
  esac
}
