...
- The former DB host `144.31.54.166` is retained for rollback copies and explicitly documented utility services — plus, since 2026-09-16 (explicit owner decision), the **isolated email-sending host**: the «Рассылка» (sender) worker `portal-worker-sender` lives there (`/opt/portal-sender`, compose `deploy/sender/docker-compose.yml`, CI target `SENDER_SSH_HOST`). Do not use the host as a current production database endpoint unless the user explicitly requests rollback verification or rollback work.
- **Email IP isolation rule (critical):** anything that makes mass outbound SMTP/IMAP connections (the sender worker, mailbox verification for рассылка) must run on `144.31.54.166` (or another dedicated host the owner names) and must NEVER be added to the `docker-compose.prod.yml` stack on the portal host — mass mailing from the portal IP gets the whole server's internet banned. Email validation stays behind `SMTP_PROXY_URLS` (proxies on 144) for the same reason. Known existing exception to review: `worker-byo-send`/`worker-byo-replies` still send direct SMTP from the portal host.
