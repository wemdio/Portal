...
case "$INSTANCE" in
  ...
  dev)
    ...
    ;;
  *)
    echo "[backup] Unknown instance: $INSTANCE (expected prod or dev)" >&2
    exit 1
    ;;
esac

DUMP_FILE="${DUMP_DIR}/instantly-${INSTANCE}-${TS}.dump"
...
if [ -n "$BACKUP_SUPABASE_URL" ] && [ -n "$BACKUP_SUPABASE_KEY" ]; then
  REMOTE_PATH="deploy-backups/instantly/${INSTANCE}/instantly-${INSTANCE}-${TS}.dump"
...
find "$DUMP_DIR" -name "instantly-${INSTANCE}-*.dump" -type f -mtime "+${RETENTION_DAYS}" -delete 2>/dev/null || true

REMAINING=$(find "$DUMP_DIR" -name "instantly-${INSTANCE}-*.dump" -type f | wc -l)
