...
**07.09.2026: релевантность с сохранением спорных контактов (код Sergey).**
Бинарное «сомневаешься — оставляй» заменено явными решениями с цитатами под
конкретную гипотезу; предварительный отказ требует второй проверки по сайту.
Недостаток сведений/ошибка не означает отказ и не даёт допуска к отправке.
Target-preview/supply сохраняют полный резерв и исходные строки. Ограниченные
автодопроверки используют сохранённое до следующего сбора, но одна неопределённая
компания не блокирует дальнейшие источники. UI разделяет готовое/резерв, даёт CSV
причин и ручную допроверку ещё не согласованного preview без пересбора/email replay.
Нужна новая миграция `20260907_0001_vertical_engine_v2_review_reserve.sql` (атомарный
RPC постановки). Старые базы/кампании автоматически не изменяются. Перенос в другие
гипотезы, повтор email validation и расширение recovery активного supply не добавлены.
Подробности, бюджеты и ограничения: [контракт резерва](design/2026-09-07-vertical-engine-v2-relevance-reserve.md),
пункт 32 журнала. Production и реальные платные вызовы не затронуты.

**07.09.2026: гонка второго supply-resume (код Sergey).** В `0512c661d` прежняя
resume-job с уже failed базой могла ошибочно считаться новой активной попыткой.
Теперь любую pending/running job проверяем до переоткрытия failed базы. Конфликт
оставляет её failed и план paused; следующая явная попытка после завершения старой
job восстанавливает очередь. Неоднозначный INSERT подтверждается точным UUID,
перед успехом перечитывается статус базы. Никаких повторных сборов/LLM в HTTP-helper
или изменений production. Подробности — пункт 31 журнала.

**07.09.2026: очистка названий компаний (код Sergey).** В новом автосборе после
email/relevance выполняется отдельная VE2-очистка названий, по уникальным компаниям,
с ограниченными пакетами и проверкой полного ответа. Raw `company` сохраняется;
`_ve_company_name` — только производное представление для preview/CSV/писем/Instantly,
не новый ключ дедупликации. Неуспешные/устаревшие названия блокируют только
соответствующие строки на границе готовой аудитории; неполная проверка останавливает
финализацию сборки. Контакты и счётчики раунда сохраняются до LLM; пакеты — после
каждого ответа. Preview можно продолжить с незавершённых названий без повторного
сбора и email validation. Supply при ошибке оставляет открытую партию на паузе;
ручное «Продолжить» восстанавливает очередь до штатных проверок активации, не
покупая новую партию. UI показывает отдельную фазу и счётчик компаний из job.
Исторические данные без метаданных и non-target/refill-задачи без checkpoints
раундов сохраняют старое поведение; они не очищаются автоматически.
Это добавляет запросы к VE_MODEL_GATE,
а не включает общий `processingSteps.clean_names` для ENG/других инструментов.
Проверки, ограничения и граница выпуска — пункт 30 журнала. Production не менялся.

**07.09.2026: исправления пятничного аудита (код Sergey).** Relevance-gate использует
локальные ID пакета и проверяет их внутри существующего ограниченного JSON-retry;
усечённый список исключений не принимается. Verdict'ы компаний сохраняются после
каждого пакета в job, при финале — в базе; новая ручная попытка читает и предыдущую
job, чтобы не потерять сохранённое при сбое финальной записи. Cache связан с точным
контекстом и моделью; legacy без checkpoint требует первой повторной проверки.
Evidence после сохранённого кандидата уступает очередь по бюджету 2 минуты / 3 кандидата;
claim сортируется по run_after. Это не параллелизм и не жёсткий двухминутный deadline
одного кандидата; cumulative usage учитывается только при полном завершении.
Досье не выбирает отрасль по «сети/центры»: ясная медицина → широкий ОКВЭД 86 с
предупреждением, неоднозначность → неизвестно. Общие каталожные фильтры не менялись.
Подробности аудита, ограничения и верификация — пункт 29 журнала. Production и
исторические результаты не изменялись; выпуск app/VE2 worker — отдельная фаза.

**07.09.2026: ошибки/восстановление превью «Приона» (код Sergey).** Requesty 402
не повторяется в рамках задачи; следующий relevance-батч после 402 не запускается.
UI сообщает причину. Ручной повтор выбирает сохранённый completed BC той же
preview-гипотезы даже за более новой пустой ошибкой; готовые строки сохраняются,
остальные проверяются без повторного сбора/обогащения/email validation.
Это замена проверки того же раунда, не повторный учёт кандидатов. Для старых
результатов нельзя обещать только незавершённые батчи: отрицательные verdict'ы
не были сохранены. Потерянную постановку можно восстановить кнопкой «Проверить
запуск», без отката возможной конкурентной job. Полные границы — пункт 28 журнала.
Миграций, изменений баланса и production-операций не было; выпуск — отдельно.

**05.09.2026: интервалы писем (код Sergey).** Defaults теперь `0,1,3,5,7,9`
дней после предыдущего письма. Шаг «Контент» показывает отдельную настройку пауз
0..90 дней, ориентир от первого, save/cancel/reset; общая защита несохранённых
правок подключена и к меню мастера. Перегенерация переносит последнюю сохранённую
схему своей вертикали после всех rewrite (включая смену языка); шаблон сохраняет
паузы исходной цепочки вместо повторного назначения defaults. Ошибка чтения
расписания прерывает перегенерацию, а не сбрасывает настройки. Старые цепочки
не переписываются автоматически. Уже строящийся/готовый шаблон нужно пересобрать
после правки пауз; существующие кампании Instantly не обновляются. Даты от первого
письма приблизительные, без учёта окна отправки. Две согласованные проверки
вызывают реальные стадии и сборщик Instantly payload; полный набор 2 355/2 355,
типы и worker build зелёные. Детали и границы браузерной приёмки: пункт 27 журнала.
Миграций и production-изменений нет.

**05.09.2026, дополнение: приёмка FlyZone / «Мармеладыч» (код Sergey).**
Реальный разбор обнаружил переформулировку «дословного» текста моделью. Теперь модель
возвращает ID начальной/конечной исходной части; сервер восстанавливает точный диапазон
и присоединяет непосредственно следующие явно помеченные рекомендации. API и БД
по-прежнему получают `text`, без служебных ID. Рекомендации не считаются фактами отрасли.
В раскрытой карточке описание показано первым, краткие поля ИИ — ниже. Это важно:
приёмка в трёх форматах подтвердила два полных исходника, но НЕ стабильную полноту
каждого AI-поля. Последний однострочный ответ опустил срок и логистику в полях, сохранив
их в исходнике; UI проверен именно на нём. Подбор кейса по рекомендациям не добавлялся,
старые записи не пересобирались, миграции/deploy не выполнялись. Детали: пункт 26 changelog.

**05.09.2026: разбор кейсов и проверка фактов перед сохранением (код Sergey).**
Ручная вставка теперь разбирается в массив самостоятельных кейсов (до 20 / 20 000
символов): preview ничего не записывает, save сохраняет выбранный набор одним
запросом без нового LLM. Новый `text` содержит исходный фрагмент конкретного кейса;
проверяются его наличие, отсутствие пересечений и числа внутри фрагмента. Оборванный
ответ не превращается в успешный неполный импорт. UI показывает поля и содержание,
даёт исключить лишнее и оставляет ввод при ошибке. Старый `text` может быть пересказом;
исходная вставка старых записей не восстанавливается. Повторный site-refresh не плодит
копии совпавших текстов и не трогает upload-кейсы. Подбор кейса для chain/template
остаётся прежним; ручной банк не подключался заново к генерации гипотез. Подробности
и ограничения: пункт 24 changelog. Миграций и изменений production не было.

**04.09.2026: защита исследования от зависшего ожидания (только код Sergey).**
LLM/сайт/DNS имеют конечное ожидание и отмену; evidence сохраняет checkpoint после
каждого кандидата и продолжает с него после сбоя. Прогресс показывает завершённые
проверки и текущий подшаг. Research execution защищён отдельным watchdog; heartbeat
не считается доказательством продвижения стадии. Подробности, подтверждённые факты
перезапуска 03.09 и ограничения:
[инцидент research stall](incidents/2026-09-03-vertical-engine-v2-research-stall.md).
Миграция не нужна; выпуск нового кода на production не выполнялся.
Отдельно остаётся неатомарное окно финализации `done → usage → enqueue next`;
данное исправление не заявляет его закрытие.
