...
**07.09.2026: повторный аудит сохранности (код Sergey, пункт 33).** После пункта 32
устранены подтверждённые потери: company-level review охватывает все сохранённые
email, факты готовой соседней строки не теряются; короткая точная деятельность
не отклоняется по длине. Дедуп/межбазовые исключения не используют одно имя и не
смешивают разные ИНН, дополняющие описания и категории сохраняются. Ручной review
после восстановления названий продолжает свою работу, а не завершается раньше.
Дополнительно введён ограниченный ручной retry незавершённой проверки отдельных
сохранённых email через validation-only child BC с durable intent/повтором того же
UUID. Нужна миграция `20260907_0002_vertical_engine_v2_saved_email_review.sql` после
`0001`; без выпуска app/worker/миграций это не изменение поведения production.
Исходные многоадресные ячейки, source-only адреса, approved/active supply не
переигрываются. Резерв не считается готовым запасом. Приёмка реальной размеченной
базы и измерение ложных допусков/отказов остаются обязательным следующим этапом.
Контракт и ограничения — [резерв релевантности](design/2026-09-07-vertical-engine-v2-relevance-reserve.md).
