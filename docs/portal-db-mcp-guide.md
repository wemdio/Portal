...
При частичном успехе блок-лист workspace не считается ошибкой всей партии. Если причины отказов полностью объясняют незагруженный остаток и известны точные `created_leads`, остальные строки становятся `skipped`, даже при нулевом `remaining_in_plan`. При смешанном отказе «место + блок-лист» агрегированные счётчики не позволяют угадать конкретные адреса: сохраняется безопасный незагруженный остаток для повторной попытки; блок-лист продолжает применяться самим Instantly. Пропуски не расходуют место: следующая часть **исходного** дневного резерва отправляется, пока место есть. `skipped` не входит в `committed_count`, поэтому не засчитывается выполнением обязательства на следующий день.

VE2 явно отправляет `skip_if_in_workspace=false`, `skip_if_in_campaign=false`, `skip_if_in_list=false` — наличие адреса в другой кампании или списке workspace не мешает загрузке. Это включает дневную доставку и старый прямой refill для проектов без delivery binding. Запрет повторной отправки уже принятых/неопределённых строк собственного плана и блок-листы остаются в силе.

## Автоматическая сверка неопределённых загрузок VE2

Миграция `20260924_0014_ve_contact_delivery_reconciliation.sql` и новый VE2 worker добавляют сверку перед обычной ежедневной доставкой (проход обычно раз в 5 минут, только проекты с активными очередями). После `uncertain` выдерживается минимум 10 минут; проверяется живой ID кампании, workspace и набор отправителей. [Instantly List Leads](https://developer.instantly.ai/api-reference/lead/list-leads) вызывается с точными `contacts` и `campaign` (не `campaign_id` на wire); адрес и принадлежность каждого результата проверяются повторно. Найденный контакт становится `accepted`. Для отсутствующего нужны две полные успешные проверки с интервалом минимум 5 минут. Ошибка, чужая кампания, неполная пагинация или устаревший результат не разрешают повторную отправку. Ошибка проверки сбрасывает отрицательное свидетельство.

`ve_contact_delivery_reconciliations` хранит отдельный журнал наблюдений: `row_id`, исходный `attempt_id/run_id`, `started_at/finished_at`, статус `checking/present/missing/released/inconclusive`, ID найденного лида и ошибку. `ve_contact_delivery_rows.recovery_token/checked_at/missing_since` защищают от двух воркеров и слишком близких проверок. Исходные массивы результатов `ve_contact_delivery_attempts` не переписываются: потерянный ответ финализации остаётся безопасно воспроизводимым. Актуальный исход после сверки читается из строк доставки и суточных счётчиков, а не только из исходного attempt. Отчётный журнал общего `client_campaign_append_batches` этой сверкой не переписывается.

Подтверждённо отсутствующая строка в тот же день возвращается в **исходный** резерв, без увеличения `reserved_count`; `recovery_retry_requested_at` разрешает воркеру продолжить этот день. Нетронутый остаток той же попытки также может продолжиться. На следующий день строка возвращается в `ready` для обычного планировщика. Пауза `upload_blocked_at` сохраняется до ручной дозаливки; сроки, согласование, исключение принятых контактов и блок-листы действуют как прежде.

Новые попытки создаёт `ve_begin_recoverable_contact_delivery`; перед каждым HTTP POST lease продлевается через `ve_renew_contact_delivery_request` на 5 минут. Запрос ограничен 90 секундами, включая тело ответа. Истёкшее владение нельзя возобновить. Если воркер пропал, после истечения lease + 5 минут попытка становится `uncertain`, затем проходит обычную сверку. Старые `attempting` без `request_lease_until` автоматически не снимаются: безопасность остановки старого процесса не доказана. Старые **финализированные** `uncertain` проверяются обычным образом. Все RPC изменения состояния доступны только service role; MCP использует только SELECT. На один проект проход обрабатывает не более 50 строк и не начинает следующую проверку после минуты работы; ответы и сохранение могут добавить время. Непроверенный остаток ждёт следующих проходов, при API-ошибке проход сверки проекта прекращается.
...
- Жизненный цикл обычных заявок: `pending` → `approved` → `paid`; вместо одобрения возможен `rejected`. После изменения 02.09.2026 новые ручные `costs` сразу создаются как `paid`: внесение означает уже совершённую оплату, без отдельной отметки согласующего.
...
- `budget_scope`: `general` для прежнего лимита разовых расходов или `costs` для отдельного лимита сервисных костов.
- `cost_category` для `budget_scope = 'costs'`: `instantly` | `email` | `bases` | `domains` | `other`; у обычных заявок поле пустое.
- У нового ручного коста `paid_on` равно введённой фактической дате (не будущей), а `expected_payment_on` хранит ту же дату для совместимости. Старые `approved`-косты остаются резервом до отдельного уточнения; автоматически считать их оплаченными нельзя.
...
**Лимит костов отдельный и общий для компании:** 650 000 ₽ каждый месяц. Он не смешивается с лимитом 75 000/40 000 ₽. В него входят оплаченные и зарезервированные заявки с `budget_scope = 'costs'`, а также строки календаря почт со статусом `keep`: до даты списания как резерв, с даты списания как факт. Валютные списания пересчитываются в рубли по последнему известному курсу не позднее даты списания; при отсутствии курса новые косты блокируются, чтобы лимит не был занижен.

### `email_subscriptions` — календарь почт и источник почтовых костов
- В лимит 650 000 ₽ попадают только строки со статусом `keep`; `active`, `pending_review` и `cancel` не учитываются.
- `billing_amount`, `currency`, `next_billing_date` определяют сумму и месяц; отрицательные суммы запрещены.
- Записи не копируются в `payment_requests`: календарь остаётся отдельным источником, а месячная сводка объединяет оба источника.
- Читать календарь могут `technician`, `lead`, `director`, `admin`; вести записи — `technician` и `admin`; принимать решения — `lead`, `director`, `admin`; удалять — только `admin`. Демо-профили доступа не имеют.
- При решении `keep` рублёвый эквивалент фиксируется, а следующая запись создаётся через 1/3/12 месяцев согласно периодичности. Старые валютные периоды без исторического курса честно остаются неполными; текущий и будущие периоды защищены жёстким лимитом.
...
После миграций `20260909_0004/0005` операционной БД:

- Обычный AI-бюджет остаётся 3, отдельный
  `reserve_instantly_qualification_ai_recovery` допускает одну финальную попытку
  на том же неизменившемся входе (суммарно ≤4). Это не дневной лимит и не счёт
  Requesty. Успешные ответы кешируются, счётчики не обнуляются при redeploy.
- `machine_reply_kind IS NOT NULL` означает доказанный технический ответ с
  `status=not_lead`, `qualified_project_id IS NULL` и
  `qualified_project_owner_proven=false`. `campaign_id` здесь — только метка
  провайдера, не доказательство проекта. Такие строки нельзя отдавать клиенту
  через текущую привязку кампании и нельзя включать в принятые лиды/уведомления.
- `recovery_failure_kind=local_read_quota` отличается от реального HTTP 429;
  короткий `recovery_next_at` — срок следующей доступности для планировщика,
  а не обещание времени доставки. `ai_final_budget_exhausted` разрешает только
  дешёвую проверку новых данных/кеша, не пятый платный запрос.
