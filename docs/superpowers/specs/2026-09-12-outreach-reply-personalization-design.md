...
| Таблица | Назначение |
|---|---|
| `reply_personalization_kb` | по одной строке на проект: факты о продукте, тон/ограничения, 1-2 примера кейсов, привязка к Instantly-аккаунту. Бриф здесь НЕ хранится (обновление 2026-09-15, миграция `20260915_0003`): он читается живьём из `projects.brief_text` — к запуску кампании бриф уже заполнен в карточке проекта, дублировать его не нужно; в модалке базы знаний бриф показывается read-only |
| `reply_personalization_global_kb` | singleton (id=1, миграция `20260916_0001`): глобальный тон/ограничения и пример хорошего письма для всех проектов. Приоритет: пер-проектное поле, если заполнено, иначе глобальное. Редактирует руководство (PUT только для supervisor-ролей), новые проекты заполнять не обязательно — глобальные значения покрывают по умолчанию |
...
3. **Сборка промпта.** Комбинация: универсальные анти-слоп правила (общие для
   ...
   проекта (бриф — живьём из `projects.brief_text`, факты о продукте, тон,
   1-2 примера из `reply_personalization_kb`) + полный тред +
...
форма: бриф проекта read-only (из карточки проекта), факты о продукте,
тон/ограничения, 1-2 примера кейса, привязка к Instantly-аккаунту/кампаниям.
Без отдельной публикации/модерации — сохранил и сразу работает. Редактировать
базу знаний проекта может любой сотрудник; отдельная роль редактора
сознательно не вводится (см. §3).

Тон/ограничения и пример письма дублируются глобально (обновление 2026-09-16):
модалка «Глобальные настройки ответов» для руководителей задаёт значения по
умолчанию для всех проектов. Поля проекта необязательны: пока поле пусто,
используется глобальное значение (в модалке проекта — синяя пометка с
превью глобального и подсказкой «заполните, чтобы переопределить»); своё
значение проекта приоритетнее глобального (пометка «переопределено»).
