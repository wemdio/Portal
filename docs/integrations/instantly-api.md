...
1. Специалист завершает сбор базы и обязательный предзапускный аудит сегментации.
2. Backend повторно проверяет exact audience, persisted assignments и freshness hash до
   первого внешнего вызова.
3. Специалист выбирает `client_campaign_presets`; пресет определяет Instantly workspace,
   mailbox IDs, расписание, лимиты и tracking-настройки.
4. Для default-группы и каждого непустого сегмента создаётся отдельная кампания.
5. `ve_templates.letters` превращаются в `sequences[0].steps`: тема, body, A/B-варианты и
   интервалы. Если create-response не вернул сохранённую sequence, Portal досылает её PATCH.
6. Проверенные строки `ve_bases` превращаются в `email + custom_variables` и загружаются
   напрямую в соответствующую кампанию через bulk import, последовательно по 1000 записей.
7. Все remote campaign IDs и количество принятых лидов сохраняются в `launch_info` и
   `ve_launch_queue_*` как один bundle.
8. Подготовка **не активирует** кампании. После QA специалист отдельно активирует весь bundle
...
Текущий UX показывает глобальный список пресетов. Пока VE-проект не связан с конкретным
клиентом/preset, специалист может выбрать не тот workspace или mailbox pool. До production
smoke это следует закрыть отдельным продуктовым изменением либо как минимум показывать в
подтверждении client/workspace/mailboxes/schedule.
