...
-- Индекс `idx_amo_leads_inn` под этот поиск живёт в отдельной миграции
-- 20260813_0002: он строится `concurrently`, а это требует применения вне
-- транзакции, тогда как здесь есть тела функций в долларовых кавычках.
-- Разделены по этой причине, а не по смыслу.
