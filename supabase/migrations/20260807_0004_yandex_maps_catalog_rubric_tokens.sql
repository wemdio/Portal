...
--
-- Отсечка по длине — не косметика, без неё индекс не строится вовсе. GIN кладёт
-- в индекс каждый токен отдельной записью, а запись не может быть больше трети
-- страницы: 2712 байт. В каталоге нашлась строка, где в «Рубрике» лежит блоб без
-- разделителей — весь он и стал одним токеном:
--
--   ERROR: index row size 2768 exceeds maximum 2712 for index "idx_ymc_rubric_tokens"
--
-- (первая постройка на бою 08.08.2026 упала на этом через 21 минуту.)
--
-- 2000 байт вместо 2712 — запас на служебные байты составного GIN «токены +
-- место». Настоящим рубрикам это не мешает: в справочник попадают только те, что
-- встречаются у 20+ организаций, а 2000 байт — это тысяча кириллических букв.
-- Отсечка одна и та же в индексе и в условии отбора (обоим считает эта функция),
-- поэтому расходиться результатам не с чем: такой «рубрики» просто нет ни там,
-- ни там.
