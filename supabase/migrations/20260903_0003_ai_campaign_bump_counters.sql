...
--
-- Ограждение жетоном — и ТОЛЬКО им, без условия на статус. p_run_token — тот же
-- жетон владения, которым библиотека ограждает все записи исполнителя в строку
-- задачи, и он один уже доказывает владение: чужой жетон в строке появиться не
-- может, пока она наша. Условие `and status = 'running'` было бы не страховкой,
-- а потерей: статус между началом звонка и записью его итога меняется законно —
-- оператор жмёт «Паузу», пока разговор идёт. Такой звонок оплачен, состоялся,
-- виден в списке контактов, а в счётчик бы не попал, и восстановить его нечем
-- (сверка счётчики не двигает). null — ручной путь из интерфейса, у которого
-- аренды нет вовсе.
--
...
returns integer
language plpgsql
-- set search_path — как у остальных plpgsql-функций репозитория: имя таблицы
-- внутри разрешается по фиксированному пути, а не по пути вызывающего.
...
begin
  update public.ai_campaigns
   ...
   where id = p_campaign_id
     and (p_run_token is null or run_token = p_run_token);
