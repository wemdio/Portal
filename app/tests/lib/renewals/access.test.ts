...
const req = (auth: string | null) =>
  ({ headers: { get: () => auth } }) as unknown as Parameters<typeof requireRenewalsAccess>[0];

describe('requireRenewalsAccess', () => {
  beforeEach(() => {
    getUser.mockReset();
  ...
  // — тот же приём, что в firstSales/access.test.ts. Работает только благодаря
  // явной аннотации возвращаемого типа в access.ts (см. комментарий там).
  const statusOf = (res: Awaited<ReturnType<typeof requireRenewalsAccess>>) =>
    'error' in res ? res.error.status : null;

  it('без токена — 401', async () => {
    expect(statusOf(await requireRenewalsAccess(req(null)))).toBe(401);
  });
  ...
  it('админ проходит без строки видимости', async () => {
    ...
    expect(statusOf(await requireRenewalsAccess(req('Bearer t')))).toBeNull();
  });
  ...
  it('обычный пользователь без строки — 403', async () => {
    ...
    single.mockResolvedValue({ data: { role: 'specialist' }, error: null });
    maybeSingle.mockResolvedValue({ data: null });
    expect(statusOf(await requireRenewalsAccess(req('Bearer t')))).toBe(403);
  });
