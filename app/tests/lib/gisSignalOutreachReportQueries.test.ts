...
describe('getSignalSlice', () => {
  it('агрегирует count компаний по сегмент × сигнал за всё время (8 сигналов)', async () => {
    ...
    expect(rows).toHaveLength(16); // 2 сегмента × 8 сигналов
    ...
    expect(rows[8]).toMatchObject({ segmentKey: 'legal', signalKey: 'signal_general_phone' });
  });
