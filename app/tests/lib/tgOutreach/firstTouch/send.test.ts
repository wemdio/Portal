...
describe('sendFirstTouchBatch', () => {
  ...
  /**
   * Регрессия 30.08.2026, моя же. Таймауты появились в боевом пути 28.08,
   * попали в общую ветку «неизвестный сбой» и начали тратить попытки контактам:
   * за трое суток 178 отложений, 33 контакта выжгли все три попытки и ушли в
   * failed. Аккаунты на медленных прокси не отправили вообще ничего — порция
   * уходила в отложенные целиком, круг за кругом.
   *
   * Таймаут на резолве — это про НАШУ связь, а не про контакт. Он обязан вести
   * себя как обрыв прокси: попытку не тратить, порцию прекратить, вернуться
   * следующим кругом.
   */
  it('таймаут резолва не тратит попытку контакта и прекращает порцию', async () => {
    const db = fakeDb([contact({ attempts: 0 }), contact({ id: 'c-2', username: 'petrov' })]);
    const client = fakeClient({ getEntity: jest.fn(() => new Promise(() => {})) });

    const res = await sendFirstTouchBatch({
      ...baseArgs, resolveTimeoutMs: 20, db, client,
    } as never);

    expect(res.sent).toBe(0);
    // Попытку не засчитали — контакт вернётся в очередь целым.
    expect(db.updates.some((u) => u.patch.attempts === 1)).toBe(false);
    // И до второго контакта не пошли: связи нет, ломиться дальше незачем.
    expect((client as unknown as { getEntity: jest.Mock }).getEntity).toHaveBeenCalledTimes(1);
  });
