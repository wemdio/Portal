...
describe('findAiOutputProblems — сравнение конкретных токенов, а не классов', () => {
  ...
  it('переформатированная моделью своя заглушка — та же заглушка, а не чужая', () => {
    // Сравнение идёт без пробелов и регистра: шаблон в гард приходит схлопнутым,
    // а модель вольна переставить пробелы внутри скобок. Придираться тут не за что.
    const tpl = 'Иван, [кейс по вашей отрасли] — показать?';
    expect(findAiOutputProblems(tpl, 'Ярослав, [ Кейс по вашей отрасли ] — показать?')).toBeNull();
  });
