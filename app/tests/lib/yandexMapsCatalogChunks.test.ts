...
describe('сбор по каталогу порциями', () => {
  it('крупная выдача идёт лесенкой и сообщает прогресс по дороге', async () => {
    ...
    expect(progress).toEqual([5_000, 10_000, 20_000, 40_000, 80_000, 160_000, 200_000, 200_000]);
    expect(mockFill.mock.calls.map((c) => c[2])).toEqual(
      [5_000, 10_000, 20_000, 40_000, 80_000, 160_000, 320_000, null],
    );
  });
  ...
  it('запрошенный потолок не превышается и не дробится сверх него', async () => {
    ...
    expect(mockFill.mock.calls.map((c) => c[2])).toEqual([5_000, 10_000, 20_000, 30_000]);
  });
});
