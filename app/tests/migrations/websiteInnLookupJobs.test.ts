...
describe('website INN lookup jobs migration', () => {
  ...
  it('persists jobs and per-row checkpoints with user isolation', () => {
    ...
    expect(sql).toMatch(/where status in \('preparing', 'pending', 'running'\)/i);
    expect(sql).toMatch(/inn_column\s*<>\s*url_column/i);
    expect(sql).toMatch(/company_column\s*<>\s*inn_column/i);
