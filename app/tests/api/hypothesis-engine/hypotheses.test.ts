...
describe('PATCH hypotheses — пересчёт вертикалей после разметки', () => {
  beforeEach(() => {
    mockDb = createMockSupabase({
      tables: {
        he_verticals: [
          { id: 'v1', project_id: 'p1', name: 'Финтех', potential_pct: 82, rank: 1 },
          { id: 'v2', project_id: 'p1', name: 'Стоматологии', potential_pct: 60, rank: 2 },
        ],
        he_hypotheses: [
          { id: 'h1', project_id: 'p1', vertical_id: 'v1', tier: 1, title: 'Банки', status: 'proposed', potential_pct: 40 },
          { id: 'h2', project_id: 'p1', vertical_id: 'v1', tier: 2, title: 'Необанки', status: 'proposed', potential_pct: 80 },
  ...
  it('принятие слабой гипотезы не топит вертикаль: % считается по всем неотклонённым', async () => {
    // Принимаем h1 (40) при живой proposed h2 (80): пул v1 = обе неотклонённые
    // → max 80 + 2 = 82. Старое правило «только принятые» уронило бы v1 до 40.
    ...
    expect(body.verticals).toEqual([
      { id: 'v1', potential_pct: 82, rank: 1 },
      { id: 'v2', potential_pct: 60, rank: 2 },
    ]);

    // Расклад не изменился — he_verticals не перезаписываем.
  ...
  it('вертикаль, где все гипотезы отклонены, получает 0% и последний rank', async () => {
    ...
    // v1 — по неотклонённым: max 80 + 2 за второго = 82.
    expect(body.verticals).toEqual([
      { id: 'v1', potential_pct: 82, rank: 1 },
    ...
    expect(rows.find((r) => r.id === 'v1')).toMatchObject({ potential_pct: 82, rank: 1 });
  });
