...
const MANUAL_OVERRIDES = {
  en: {
    ...
    'Отправки, ответы и лиды смотрите в разделе': 'View sends, replies, and leads in',
    // Открытия убраны из клиентской статистики (2026-09-22): эти строки
    ...
    'Переписку сейчас не удалось загрузить полностью — повторите через минуту.': 'Could not load the full conversation right now — try again in a minute.',
    'Воронка:': 'Funnel:',
  ...
  es: {
    ...
    'Переписку сейчас не удалось загрузить полностью — повторите через минуту.': 'No se pudo cargar la conversación completa ahora; inténtalo de nuevo en un minuto.',
    'Воронка:': 'Embudo:',
