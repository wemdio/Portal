...
\echo '=== 1b. Заглушки [в квадратных скобках] от GPT в отправленных (должно быть 0) ==='
-- Инцидент 19.08.2026: на пустой истории модель сочинила из 300-знакового шаблона
-- деловое письмо с подписью и оставила `[Ваше имя]`, `[Контактная информация]`.
-- Ушло человеку из Oracle. Пункт 1 это не ловит: там фигурные скобки, тут квадратные,
-- а в дайджесте обе отправки записались как успешная персонализация.
SELECT COUNT(*) AS bracket_placeholder_msgs
FROM li_leads l
CROSS JOIN LATERAL jsonb_array_elements(COALESCE(l.conversation_history, '[]'::jsonb)) AS m
WHERE m->>'role' = 'assistant' AND (m->>'content') ~ '\[[^\]]{2,60}\]'
  AND CASE WHEN m->>'ts' ~ '^\d{4}-\d{2}-\d{2}T' THEN (m->>'ts')::timestamptz END
...
-- бы вечно) и живой рецидив тонет в историческом фоне. То же окно у пункта 1b.
-- Пункт 1 без окна намеренно: сырых {{...}} в базе нет вовсе, копить нечему,
-- а если появятся — это должно быть слышно сразу и не затихать через две недели.
