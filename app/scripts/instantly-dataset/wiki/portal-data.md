...
| `portal_contacts_history` | дневные снапшоты факта | ЕДИНСТВЕННЫЙ типизированный КПИ-источник (int). Темп = дельта за окно ≤90 последних точек (`v_portal_project_pace`). ⚠️ `period_id` в источнике пока не заполняется → у проектов с АКТИВНЫМ периодом (продлённых) темп/прогноз = NULL (как и в UI); судить о них по `leads_30d`/задачам |
