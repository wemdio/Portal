...
| `portal_contacts_history` | дневные снапшоты факта | ЕДИНСТВЕННЫЙ типизированный КПИ-источник (int). Темп = дельта за окно ≤90 последних точек (`v_portal_project_pace`). `period_id` реально пишется только с 2026-07-06 (до этого снапшоттер падал на 42P10 и period-строки терялись); бэкфиллены 3 активных периода, у остальных история периода стартует 2026-07-07+ |
