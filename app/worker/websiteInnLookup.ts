 ...
 * processed. Фаз, в которых колонка законно стоит, три:
 ...
 *    секунд;
 *  - САМАЯ ДЛИННАЯ — ожидание чужих строк в конце задачи. Когда очередь
 *    pending пуста, но чьи-то строки ещё числятся «в работе», тело ждёт, пока
 *    они дорастут до ITEM_STALE_MS (2 мин), и только потом возвращает их себе
 *    (websiteInnLookupRunner.ts). Всё это время processed стоит: 2 мин
 *    ожидания + пачка ≈ 3 минуты.
 * То есть законный максимум ≈ 3 минуты.
 *
 ...
 *     порог простоя (10 мин, отсчёт начинается сразу: значение колонки читается
 *     уже при захвате, см. select ниже)
 ...
 * Без select колонки прогресса к сумме добавился бы ещё один тик продления
 * (60 с) на её засев — 15,5 мин, тоже влезает, но запас съедается зря.
 * Запас троекратный снизу и пятиминутный сверху. Поднимать порог монитора для
...
async function main(): Promise<void> {
  ...
  const runner = createJobRunner<{ id: string }, { processed: number }>({
    ...
    // иначе первый тик продления потратился бы на её засев, и детектор простоя
    // включался бы на минуту позже (арифметика — над STALL_MS).
    select: 'processed',
    /**
     * ВЕСЬ бюджет остановки — 5 секунд, а не «5 секунд плюс освобождения».
     * Библиотека заводит один таймер на всю остановку, и два прохода
     * освобождения аренды с паузой в секунду и контрольное чтение идут ВНУТРИ
     * него; телу достаётся остаток. Складывать слагаемые здесь — ошибка,
     * унаследованная из прежних комментариев (её же поправили в задаче про
     * валидацию почт этого этапа). Пять секунд с огромным запасом влезают и в
     * stop_grace_period 30 с, и в docker stop --timeout 15 от деплоя.
     *
     * Дольше ждать тело бессмысленно: оно смотрит на signal на границе строки,
     * а один обход сайта идёт до 44 с — пачку оно всё равно не доиграет, зато
     * успеет вернуть её строки в очередь, и их проверит сосед.
     */
