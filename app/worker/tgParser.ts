...
async function main(): Promise<void> {
  ...
  const runner = createJobRunner<{ id: string }, TgParserCheckpoint>({
     ...
     * его промис не осядет, owned.delete в finally не выполнится — и pollOnce
     * будет вечно упираться в предел параллелизма. То есть этот контейнер
     * структурно не способен взять брошенную строку обратно, а другого тут
     * нет (реплика одна, см. ниже): задача осталась бы в running с протухшей
     * арендой, и взять её было бы некому.
     *
     ...
     *    поэтому отпустить аренду по простою просто некому в руки: соседа,
     *    который подхватил бы строку, не существует, а этот контейнер её не
     *    возьмёт (слот занят неосевшим промисом, см. выше). Опция дала бы не
     *    починку, а брошенную строку и потраченную попытку;
