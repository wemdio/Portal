...
async function main(): Promise<void> {
  ...
  const runner = createJobRunner<{ id: string }, { processed: number }>({
     ...
     *
     * Формулировка «ждём тело 5 с» из шаблона поиска (worker/search.ts)
     * неточна, и это неточность уровня шаблона: таймер бюджета заводится ДО
     * освобождений, а из него последовательно тратятся два прохода обнуления
     * аренды с паузой в секунду между ними и контрольное чтение строк. То есть
     * тело получает примерно две секунды, а не пять.
     *
     * Больше и не нужно: тело смотрит на signal на границе строки очереди, а
     * одна SMTP-проба идёт до 25 с — за любой разумный остаток оно пачку не
     * доиграет, и продолжит её сосед с тех же строк. Значение выбрано так,
     * чтобы весь цикл остановки уложился в stop_grace_period 30 с с запасом.
     */
