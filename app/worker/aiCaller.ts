...
async function dispatchCommand(job: { id: string; campaign_id: string; action: string }): Promise<void> {
  ...
  switch (job.action) {
    ...
    case 'stop': {
       ...
       *
       * Статус `paused` интерфейс пишет сам ещё до постановки команды; здесь мы
       * добиваем случай, когда он этого не сделал, и главное — снимаем
       * владение. Исполнитель узнает об остановке двумя путями: его продление
       * аренды перестанет находить строку в `running` (≤ треть аренды, 60 с) и
       * цикл перечитает статус на ближайшем круге. Ждать конца цикла здесь
       * нельзя: тело может идти в другой реплике.
       */
      const { error } = await db
        .from('ai_campaigns')
        .update({
