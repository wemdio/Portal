...
const campaignRunner = createJobRunner<{ id: string; called_contacts: number }, unknown>({
  ...
  /*
   * Весь бюджет остановки: 8 секунд.
   *
   * Деплой останавливает контейнер `docker compose stop --timeout 15`. В эти
   * восемь секунд библиотека укладывает два прохода освобождения аренды с
   * паузой в секунду между ними и контрольное чтение (около трёх), а остаток —
   * около пяти — достаётся телу. Телу и нужно куда меньше: все паузы
   ...
   * дописывая. До семи секунд деплоя остаётся в резерве.
   *
   * beforeRelease здесь не нужен: дорогих внешних подключений у обзвона нет,
   * звонок ведёт провайдер, и пересечение владельцев стоит в худшем случае
   * одного лишнего круга по контактам — от него защищает атомарный захват
   * контакта, а не порядок остановки.
   */
