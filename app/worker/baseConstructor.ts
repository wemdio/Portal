 ...
 *  - порог простоя (10 мин) — движется ли САМА РАБОТА. Продление таймером
 ...
 *
 * ВРЕМЯ ДО ПЕРЕХВАТА СКЛАДЫВАЕТСЯ ИЗ ТРЁХ СЛАГАЕМЫХ, и сравнивать с порогом
 * монитора «Долго висит» (HEALTH_JOB_STUCK_MIN, 20 мин) надо именно сумму:
 *
 *     порог простоя (10 мин)
 *   + не больше одной аренды (5 мин: продление прекращается, и аренда
 *     доживает свой срок сама)
 *   + не больше одного опроса соседа (30 с: realtime будит только на
 *     status=pending, зависшая processing-строка никого не будит)
 *   ≈ 16 минут в худшем случае.
 *
 * Прежние 15 минут (BASE_CONSTRUCTOR_STALE_MINUTES) были ВСЕМ этим временем
 * целиком, а не первым слагаемым: с ними сумма выходила 20-22 минуты, и
 * монитор успевал поднять тревогу за секунду до самопочинки — ровно тот
 * сценарий, ради которого порог монитора и подняли до 20 (инцидент
 * 11.08.2026).
 *
 * Ниже 10 опускать не стоит, выше — незачем. Проверено по шагам: find_emails
 * и enrich двигают прогресс каждые 10 обработанных строк (пул 10 и 6 при
 * жёстких таймаутах 15 с и 60 с на сайт — это минуты, не десятки минут),
 * validate_emails стучится каждые 30 с даже во время ожидания, AI-шаги
 * отчитываются попачечно. Дольше 10 минут молчит только патологический
 * retry-шторм у AI-провайдера (до 4 попыток × 70 с таймаута плюс паузы
 * лимита), но это и есть застрявшая работа: её честнее перехватить и
 * продолжить с чекпоинта, чем ждать.
 */
const STALL_MS = Math.max(
  60_000,
  Number(process.env.BASE_CONSTRUCTOR_STALL_MINUTES ?? '10') * 60_000,
);
