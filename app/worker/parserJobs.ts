 ...
 *    ≥ 5,7 мин и до часа на пункт. У ATS худший законный промежуток мал (25
 *    бордов × 15 с таймаута ≈ 6 мин), но раннер один на набор типов и настройка
 *    у него одна.
 *
 * Ложное срабатывание тут стоит дороже пропуска: аренда бросается, соседняя
 * реплика берёт задачу и гоняет её С НУЛЯ — и так по кругу, пока законный этап
 * не уложится в порог. Поэтому живость РАБОТЫ здесь оставлена внешнему
 * наблюдателю: монитор здоровья (services/health-check/main.py, спецификация
 * parser_jobs) считает простой по отпечатку из пяти колонок прогресса и шлёт
 * алерт «Долго висит» через 20 минут. Продление аренды в этот отпечаток не
 * входит, так что наблюдение честное. Библиотека же ловит смерть ПРОЦЕССА —
 * ровно то, ради чего сюда и приехала.
 */
