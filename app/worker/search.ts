...
async function main(): Promise<void> {
  ...
  const runner = createJobRunner<{ id: string }, { processed_queries: number }>({
     ...
     * а запросов параллельно всего два (SEARCH_CONFIG.QUERY_CONCURRENCY = 2,
     * литерал в lib/config.ts, переменной окружения нет; единица в
     * searchParserWorker.ts — лишь фолбэк toPositiveInt на негодное значение).
     * Один запрос законно идёт десятками минут:
