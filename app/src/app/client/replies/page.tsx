...
type ForwardedLead = {
  ...
  /** «Сирота»: провайдер не привязал письмо к кампании (атрибуция по домену) — бейдж «вне треда». Тред и ответ работают (право по ящику — strayAccess; отправка через fallback — reply/route.ts). */
...
function LeadDetail({
  ...
  // Отвечать по ней МОЖНО — роут /reply при отказе провайдера уходит на отправку
  // новым письмом тем же ящиком (см. комментарий в reply/route.ts). Отдельного
  // гейта на ответ нет: подсказка «ответьте сами из ящика» была бы невыполнимой,
  // отправляющие ящики наши и доступа к ним у клиента нет.
  //
  // Признак «ящик наш» — ровно наличие `eaccount`: список подставляет его ТОЛЬКО
  // для собственного ящика клиента (route.ts, ownMailbox), а сироту с чужим
  // ящиком показывает без него — по ней и тред, и ответ честно отдают 404, так
  // что действий на ней не рисуем.
  ...
  const canReplyByEmail = canViewThread;
        ...
        {canViewThread && (
          <div className="mb-2">
            ...
            <ExpandedThread
              ...
              className="space-y-3"
            />
            {/* Сирота: ответ уходит отдельным письмом, а не в тред, поэтому в
                ленте он не появится. Предупреждаем заранее — иначе выглядит так,
                будто отправка не сработала. */}
            {isOutOfCampaign && (
              <p className="text-xs mt-3" style={{ color: 'var(--cp-text-l)' }}>
                Письмо пришло вне треда кампании: ваш ответ уйдёт адресату отдельным
                письмом и в этой переписке не отобразится.
              </p>
            )}
          </div>
        )}
