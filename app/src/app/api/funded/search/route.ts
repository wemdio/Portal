...
export async function GET(req: NextRequest) {
  ...
  // No COUNT(*) here — the counter is /api/funded/count (exact count over the
  // same filter chain, see lib/funded/queryFilters); export loops until a page
  // returns fewer than `limit`.
  // (cast as never: the concrete postgrest generic is too deep for tsc; the
  // helper mutates the builder in place)
  const query = supabase.from('funded_companies').select('*');
