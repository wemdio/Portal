...
export const GET = withTgOutreachAuth(async (_req, { supabase }, params) => {
  ...
  const junctions = (data.tg_outreach_proxy_tags ?? []) as Array<{ tg_outreach_tags: unknown }>;
...
export const PATCH = withTgOutreachAuth(async (req, { supabase }, params) => {
  ...
  if (body.tag_ids !== undefined) {
    await supabase.from('tg_outreach_proxy_tags').delete().eq('proxy_id', id);
...
export const DELETE = withTgOutreachAuth(async (_req, { supabase }, params) => {
  const id = params?.id;
  const { error } = await supabase.from('tg_outreach_proxies').delete().eq('id', id);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
