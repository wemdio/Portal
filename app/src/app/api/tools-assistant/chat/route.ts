...
function buildSearchSection(hits: SearchHit[]): string {
  if (hits.length === 0) {
    return 'КОНТЕКСТНЫЙ ПОИСК (real-time, по исходникам инструментов):\n- По последнему вопросу пользователя в исходниках портала ничего конкретного не нашлось. Значит, такой кнопки/функции в портале, скорее всего, нет — не выдумывай инструмент «на похожую тему», честно скажи, что не нашёл.';
  }
  const lines: string[] = [];
  lines.push('КОНТЕКСТНЫЙ ПОИСК (real-time, по исходникам инструментов):');
  lines.push('- По последнему вопросу пользователя в исходниках портала нашлись следующие инструменты с совпавшими элементами интерфейса:');
  ...
  lines.push('- Используй именно эти инструменты для ответа. Не подсовывай вместо них что-то «на похожую тему».');
  if (hits.length > 1) {
    lines.push('- Совпадений несколько — ситуация неоднозначная. НЕ выбирай один инструмент сам. Вместо этого ответь короткой сводкой: «У нас есть несколько инструментов с похожей функцией, какой из них вам нужен?» — и кратко перечисли все варианты (название + 1 фраза о назначении + ссылка). Дай пользователю выбрать, не отвечай развёрнуто ни по одному из них, пока он не уточнит.');
  } else {
    lines.push('- Совпадение одно — спокойно отвечай по нему (раздел, блок, ссылка, краткое описание кнопки).');
  }
  lines.push('- Это поиск по реальному коду, поэтому он надёжнее каталога ниже. Если совпавший лейбл — название кнопки, упоминай его дословно в ответе.');
  return lines.join('\n');
}
...
export async function POST(req: NextRequest) {
  ...
  // Реал-тайм поиск по исходникам инструментов: для каждого ответа берём
  // последний user-вопрос и ищем в индексе UI-лейблов. Результаты попадут в
  // системный промпт перед моделью.
  const searchHits = searchPortalUi(messages[messages.length - 1].content);
