...
const PARSER_TYPE = 'polza_outreach' as const;
...
async function getSupabase(req: NextRequest) {
  ...
  try {
    ...
    if (error || !data?.user) {
      await logError('parser.polza_outreach.auth.failed', error ?? 'User not found', { hasUser: Boolean(data?.user) }, logMeta);
      return { error: jsonError('Unauthorized', 401, { request_id: requestId }) };
    }
    return { supabase, user: data.user };
  } catch (err) {
    await logError('parser.polza_outreach.auth.exception', err, undefined, logMeta);
    return { error: jsonError('Unauthorized', 401, { request_id: requestId }) };
  }
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    raw = (await req.json()) as Partial<PolzaOutreachConfig>;
  } catch {
  ...
  const config = sanitizePolzaOutreachConfig(raw);
  const { data, error } = await supabase
    .from('parser_jobs')
    .insert({
      user_id: user.id,
  ...
  if (error) {
    await logError('parser.polza_outreach.job.create.failed', error, { parserType: PARSER_TYPE }, logMeta);
    return jsonError(error.message, 500, { request_id: requestId });
  }

  await logAudit('parser.polza_outreach.job.created', 'Polza outreach parser job created', { jobId: data?.id, config }, logMeta);
