...
export async function POST(
  ...
  try {
    ...
    const sys = [
      ...
      '- Письмо 4: ещё один короткий заход с НОВЫМ углом или ценностью, лёгкий FOMO без давления, простой следующий шаг.',
      '- ЗАПРЕЩЕНЫ «прощальные»/breakup-письма — даже если примеры ниже так делают (это маркер массового спама): НИ ОДНО письмо не объявляет себя последним, не просит ответить «не нужно»/«не интересно», не обещает «больше не буду беспокоить/писать», не извиняется за беспокойство и не упоминает отписку.',
      '',
