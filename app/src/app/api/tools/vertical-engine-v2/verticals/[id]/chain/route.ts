...
import { defaultChainLanguageForMarket, projectMarket } from '@/lib/verticalEngineV2/market';
import type { VeChainLanguage } from '@/lib/verticalEngineV2/types';
...
const LANGUAGES: VeChainLanguage[] = ['ru', 'en', 'pl'];
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  return withToolTrace(
    ...
    async () => {
      ...
      let language: VeChainLanguage | null = null;
      try {
        const body = (await req.json()) as { language?: unknown };
        if (body?.language !== undefined) {
          if (typeof body.language !== 'string' || !LANGUAGES.includes(body.language as VeChainLanguage)) {
            return jsonError('language должен быть ru, en или pl', 400);
          }
          language = body.language as VeChainLanguage;
        }
      } catch {
      ...
      const { data: job, error: jobErr } = await supabaseAdmin
        ...
        .single();
      if (jobErr || !job) {
        await logError('tools.vertical-engine-v2.chain.enqueue_failed', jobErr, { userId, verticalId: id });
        return jsonError(jobErr?.message ?? 'Не удалось поставить задачу', 500);
      }

      void logAudit('tools.vertical-engine-v2.chain.enqueued', 'Hypothesis engine chain enqueued', {
        userId,
        verticalId: id,
        language,
        jobId: job.id,
