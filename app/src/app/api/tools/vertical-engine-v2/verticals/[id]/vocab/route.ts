...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  return withToolTrace(
    { request: req, operation: 'tools.vertical-engine-v2.vocab.post' },
    async () => {
      ...
      const { data: job, error: jobErr } = await supabaseAdmin
        ...
        .single();
      if (jobErr || !job) {
        await logError('tools.vertical-engine-v2.vocab.enqueue_failed', jobErr, { userId, verticalId: id });
        return jsonError(jobErr?.message ?? 'Не удалось поставить задачу', 500);
      }

      void logAudit('tools.vertical-engine-v2.vocab.enqueued', 'Hypothesis engine vocab enqueued', {
        userId,
        verticalId: id,
        jobId: job.id,
      });
