...
import {
  patchVeProjectBrief,
  type VeBriefPatchBody,
} from '@/lib/verticalEngineV2/projectBriefPatch';
...
// lib/verticalEngineV2/projectDetail.ts (её же использует клиентский
// ENG-контур со скоупом владельца).
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  return withToolTrace(
    { request: req, operation: 'tools.vertical-engine-v2.projects.detail' },
    async () => {
      ...
      const result = await loadVeProjectDetail(supabaseAdmin, id);
      if (!result.ok) {
        return jsonError(
...
// ve_projects.brief и уточняют генерацию цепочек. Логика — в
// lib/verticalEngineV2/projectBriefPatch.ts (её же использует клиентский
// ENG-контур); здесь — auth и RU-тексты ошибок.
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  return withToolTrace(
    { request: req, operation: 'tools.vertical-engine-v2.projects.patch' },
    async () => {
      const authed = await requireInternalToolAuth(req);
      if ('error' in authed) return authed.error;
      if (!supabaseAdmin) return jsonError('Server misconfigured', 500);
      ...
      let body: VeBriefPatchBody;
      try {
        body = (await req.json()) as VeBriefPatchBody;
      } catch {
        return jsonError('Invalid body', 400);
      }

      const result = await patchVeProjectBrief(supabaseAdmin, id, body);
      if (!result.ok) {
