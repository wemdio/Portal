...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  return withToolTrace(
    { request: req, operation: 'tools.vertical-engine-v2.bases.post' },
    async () => {
      ...
      const { data: base, error: baseErr } = await supabaseAdmin
        ...
        .single();
      if (baseErr || !base) {
        await logError('tools.vertical-engine-v2.bases.insert_failed', baseErr, { userId, projectId: id });
      ...
      if (jobErr) {
        await logError('tools.vertical-engine-v2.bases.enqueue_failed', jobErr, {
          userId,
          projectId: id,
          baseId: base.id,
        });
      ...
      void logAudit('tools.vertical-engine-v2.bases.uploaded', 'Hypothesis engine base uploaded', {
        userId,
        projectId: id,
        baseId: base.id,
        verticalId,
