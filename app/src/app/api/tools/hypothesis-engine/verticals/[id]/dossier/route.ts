...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  return withToolTrace(
    { request: req, operation: 'tools.hypothesis-engine.dossier.post' },
    async () => {
      ...
      const { data: job, error: jobErr } = await supabaseAdmin
        ...
        .single();
      if (jobErr || !job) {
        await logError('tools.hypothesis-engine.dossier.enqueue_failed', jobErr, { userId, verticalId: id });
        return jsonError(jobErr?.message ?? 'Не удалось поставить задачу', 500);
      }

      void logAudit('tools.hypothesis-engine.dossier.enqueued', 'Hypothesis engine dossier enqueued', {
        userId,
        verticalId: id,
        jobId: job.id,
      });
