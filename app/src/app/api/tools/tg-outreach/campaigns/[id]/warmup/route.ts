...
export async function POST(req: NextRequest, ctx: Ctx) {
  return withToolTrace(
    ...
    async () => {
      ...
      /**
       * Запущенная кампания прогреву больше не мешает.
       *
       * Взаимоисключающими остаются аккаунт и его роль, а не кампания целиком:
       * греется тот, у кого проставлен `warmup_until`, а боевой круг таких
       * пропускает (миграция 20260907_0001). Прежний отказ останавливал
       * рассылку клиенту ради прогрева новой партии — поэтому прогревом и не
       * пользовались: один запуск у ATOL-1 за всё время, ноль у Polza.
       */
      ...
      /**
       * Статус кампании не трогаем.
       *
       * Раньше он и означал «идёт прогрев» — и потому запирал всё остальное:
       * кампанию нельзя было ни запустить, ни остановить, пока прогрев не
       * кончится. Теперь греется аккаунт, а не кампания, и та продолжает
       * рассылать теми, кого не отметили. Идущий прогрев виден на своей вкладке
       * и восклицательным знаком у статуса.
       */
