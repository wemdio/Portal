...
import {
  ChevronLeft, Loader2, ListChecks, Plus, Search,
} from 'lucide-react';
...
export default function LeadListsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      const data = await instantlyFetch<{ items: LeadList[] }>('/lead-lists?limit=all');
      setLists(data.items ?? []);
    } catch (err) {
  ...
  const handleCreate = useCallback(async () => {
    ...
    setCreating(true);
    try {
      const list = await instantlyFetch<LeadList>('/lead-lists', {
        method: 'POST',
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8">
      ...
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-zinc-900">Lead списки</h1>
        <button
        ...
        >
          <Plus className="h-4 w-4" /> Создать список
        </button>
      </div>

      {showCreate && (
        <div className="mb-5 rounded-xl border border-zinc-200 bg-white p-5">
          <h3 className="mb-3 text-sm font-semibold text-zinc-900">Новый список</h3>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-zinc-200 bg-white py-16 text-center">
          <ListChecks className="mx-auto h-8 w-8 text-zinc-300" />
          <p className="mt-3 text-sm text-zinc-500">{search ? 'Ничего не найдено' : 'Нет списков'}</p>
        </div>
      ) : (
        <div className="rounded-xl border border-zinc-200 bg-white divide-y divide-zinc-100">
          {filtered.map((l) => (
            <Link
              key={l.id}
              href={`/instantly/leads?lead_list_id=${l.id}` as Route}
