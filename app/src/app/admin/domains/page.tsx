...
import { RefreshCw, AlertTriangle, Globe, ExternalLink, Search, ChevronLeft, ChevronRight, X } from 'lucide-react';
...
export default function AdminDomainsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (q) {
      result = result.filter((d) =>
        d.dname.toLowerCase().includes(q) || d.account.toLowerCase().includes(q),
      );
    }
    return result;
  ...
  // на друга. Домен-колонка — flex, остальные фиксированные.
  const gridCols = 'sm:grid-cols-[minmax(140px,1fr)_110px_100px_100px_150px_130px]';

  return (
    <div className={`max-w-7xl mx-auto px-4 ${isTma ? 'py-6 text-sm leading-relaxed' : 'py-10'}`}>
      <div className="mb-6">
          ...
          {domains.length > 0 && (
            <div className="relative mb-3">
              ...
              <input
                ...
                placeholder="Поиск по домену или аккаунту…"
                className="w-full rounded-lg border border-gray-200 bg-white pl-9 pr-9 py-2 text-sm outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100 transition"
              />
              {search && (
                <button
                  type="button"
            ...
            <div className="flex items-center justify-between gap-3 mt-4">
              ...
              <div className="flex items-center gap-1">
                <button
                  ...
                  className="inline-flex items-center justify-center rounded-md border border-gray-200 bg-white p-1.5 text-gray-600 hover:bg-gray-50 disabled:opacity-30 disabled:cursor-not-allowed"
                  aria-label="Предыдущая страница"
                >
                  <ChevronLeft className="h-4 w-4" />
                </button>
                <span className="text-xs text-gray-600 px-2 tabular-nums min-w-[3rem] text-center">
