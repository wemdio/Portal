...
// «Гипотезы по сбору базы» под брифом (GET /api/client/brief/hypotheses).
// Markdown — рендерится LeadSourceHypothesesView. Источники и сегменты
// консистентны с демо-кампаниями (розница / 3PL / производство / маркетплейсы)
// и инструментами портала (поиск по вакансиям, Яндекс.Карты, поиск, Конструктор баз).
// Формат строго под parseHypotheses: секции «### Гипотеза N: …» + поля
// «- Лейбл: значение». Плоские абзацы без буллетов парсер ВЫБРАСЫВАЕТ
// (рендерится одна битая карточка) — не «упрощать» обратно в свободный текст.
export const DEMO_BRIEF_HYPOTHESES_RESPONSE = {
  lead_source_hypotheses:
    '### Гипотеза 1: Поиск по вакансиям — розничные сети, которые нанимают складской персонал\n' +
    '- Источник: поиск по вакансиям\n' +
    '- Фильтры/запросы: «кладовщик», «руководитель склада», «специалист по инвентаризации» — по ритейлу с несколькими филиалами\n' +
    '- Почему сработает: компания активно нанимает складской персонал — значит, склады есть и растут\n' +
    '- Ожидаемый объём: 600–900 компаний\n\n' +
    '### Гипотеза 2: Яндекс.Карты — распределительные центры и склады ритейла\n' +
    '- Источник: парсер Яндекс.Карт\n' +
    '- Фильтры/запросы: «распределительный центр», «склад розничной сети» по Москве, МО и городам-миллионникам\n' +
    '- Ожидаемый объём: 400–700 организаций с сайтами и контактами\n\n' +
    '### Гипотеза 3: Яндекс.Карты — 3PL и фулфилмент-операторы\n' +
    '- Источник: парсер Яндекс.Карт\n' +
    '- Фильтры/запросы: «фулфилмент», «3PL оператор», «ответственное хранение»\n' +
    '- Почему сработает: прямо под сегмент кампании «3PL и фулфилмент»\n' +
    '- Ожидаемый объём: 300–500 компаний\n\n' +
    '### Гипотеза 4: Поиск — продавцы маркетплейсов с несколькими складами\n' +
    '- Источник: поисковый парсер\n' +
    '- Фильтры/запросы: «фулфилмент для Wildberries», «отгрузки Ozon поставщик», каталоги технологических партнёров маркетплейсов\n' +
    '- Почему сработает: под сегмент кампании «продавцы на маркетплейсах»\n' +
    '- Ожидаемый объём: 500–800 компаний\n\n' +
    '### Гипотеза 5: Конструктор баз — выгрузка по ОКВЭД с AI-скорингом\n' +
    '- Источник: Конструктор баз\n' +
    '- Фильтры/запросы: ОКВЭД 47.x (розничная торговля) и 52.10 (складирование) с фильтром по выручке; дальше дедупликация, поиск почт, оценка по брифу\n' +
    '- Ожидаемый объём: 1 500–2 500 целевых компаний после чистки\n\n' +
    '### Гипотеза 6: Поиск по вакансиям — производственные компании с логистикой\n' +
    '- Источник: поиск по вакансиям\n' +
    '- Фильтры/запросы: вакансии «логист», «начальник склада» у производственных компаний (ОКВЭД 10–32)\n' +
    '- Почему сработает: под сегмент кампании «производственные компании»\n' +
    '- Ожидаемый объём: 800–1 200 компаний',
  lead_source_hypotheses_generated_at: '2026-04-28T12:05:00.000Z',
