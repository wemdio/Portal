...
export const DEMO_BRIEF_HYPOTHESES_RESPONSE = {
  lead_source_hypotheses:
    '**1. HH.ru — розничные сети, которые нанимают складской персонал**\n' +
    'Источник: парсер HH.ru. Запросы: «кладовщик», «руководитель склада», «специалист по инвентаризации» с фильтром по ритейлу с несколькими филиалами. Компания активно нанимает склад — значит, склады есть и растут. Ожидаемый объём: 600–900 компаний.\n\n' +
    '**2. Яндекс.Карты — распределительные центры и склады ритейла**\n' +
    'Источник: парсер Яндекс.Карт. Запросы: «распределительный центр», «склад розничной сети» по Москве, МО и городам-миллионникам. Ожидаемый объём: 400–700 организаций с сайтами и контактами.\n\n' +
    '**3. Яндекс.Карты — 3PL и фулфилмент-операторы**\n' +
    'Запросы: «фулфилмент», «3PL оператор», «ответственное хранение». Прямо под сегмент кампании «3PL и фулфилмент». Ожидаемый объём: 300–500 компаний.\n\n' +
    '**4. Поиск — продавцы маркетплейсов с несколькими складами**\n' +
    'Источник: поисковый парсер. Запросы: «фулфилмент для Wildberries», «отгрузки Ozon поставщик», каталоги технологических партнёров маркетплейсов. Ожидаемый объём: 500–800 компаний под сегмент «продавцы на маркетплейсах».\n\n' +
    '**5. Конструктор баз — выгрузка по ОКВЭД с AI-скорингом**\n' +
    'Выгрузка компаний по ОКВЭД 47.x (розничная торговля) и 52.10 (складирование) с фильтром по выручке, дальше через Конструктор: дедупликация, поиск почт, оценка по брифу. Ожидаемый объём после чистки: 1 500–2 500 целевых компаний.\n\n' +
    '**6. HH.ru — производственные компании с логистикой**\n' +
    'Вакансии «логист», «начальник склада» у производственных компаний (ОКВЭД 10–32). Под сегмент кампании «производственные компании». Ожидаемый объём: 800–1 200 компаний.',
