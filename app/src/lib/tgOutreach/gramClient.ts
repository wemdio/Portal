...
export interface BuildClientsOptions {
  ...
  signal?: AbortSignal;
  /**
   * Отметка «работа идёт» на КАЖДЫЙ аккаунт.
   *
   * Подключение — самая долгая молчаливая фаза цикла: на аккаунт уходит до
   * минуты с небольшим (таймаут подключения, TCP-проверка прокси, повторная
   * попытка), и на кампании в три десятка аккаунтов лежащие прокси растягивают
   * проход за полчаса. Пока отсюда не шло ни одной отметки, порог простоя
   * (25 минут) считал это зависанием: аренда уходила, сокеты рвались, кампанию
   * перезахватывали — и после трёх таких кругов она вставала с диагнозом
   * «исполнитель терял задачу три раза». Диагноз неверный: терялись не задачи,
   * а прокси, и по журналу этого было не понять.
   *
   * Рядом уже пишется heartbeat контейнера — по той же причине и в той же
   * точке; эта отметка нужна отдельно, потому что живой процесс и движущаяся
   * работа — разные вещи.
   */
  onProgress?: () => void;
}
