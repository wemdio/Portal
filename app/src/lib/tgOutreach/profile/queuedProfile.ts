 ...
 * аккаунтами в эти часы можно и нужно — именно этим пауза и наполняется.
 *
 * Двенадцать часов, а не сутки из руководства: партия настраивается вечером и к
 * утру должна быть в работе. Прогрев за это время всё равно идёт, так что
 * аккаунт не простаивает, а разница между «переименовали и сразу пишем» и
 * «переименовали, полдня пожили, потом пишем» для Telegram уже есть.
 *
 * Описание в правило не входит намеренно: правка биографии не меняет того, как
 * аккаунт выглядит в списке контактов, и терять из-за опечатки полдня не за что.
 */
export const PROFILE_REST_HOURS = Number(process.env.TG_OUTREACH_PROFILE_REST_HOURS) || 12;
