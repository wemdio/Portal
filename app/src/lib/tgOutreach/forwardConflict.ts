...
/**
 * Можно ли снять передачу из очереди — и если нет, то почему.
 *
 * Между нажатием кнопки и отправкой проходят часы: отправляет не портал, а
 * воркер, когда дойдёт до аккаунта в круге. Всё это время ошибочно поставленную
 * задачу ещё можно остановить, и это единственное окно — после отправки
 * сообщение уже у менеджера, и «отмена» означала бы только ложную уверенность
 * оператора, что адресат ничего не видел.
 */
export function cancelBlockReason(forward: ExistingForward | null): string | null {
  if (!forward) return 'Этой передачи нет в очереди — отменять нечего';

  if (forward.status === 'sent') {
    return `Передача уже ушла менеджеру (${label(forward.kind)}) — отозвать сообщение из Telegram нельзя`;
  }

  // Упавшая и снятая уже не в очереди: воркер их не возьмёт, и снимать нечего.
  // Отказ здесь не тупик — обе разблокируют кнопки «Передать», и оператор
  // поставит задачу заново.
  if (forward.status !== 'pending') {
    return 'Эта передача уже не в очереди — отменять нечего';
  }
