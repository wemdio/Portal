 ...
 * строку.
 *
 * Порог 600 знаков — один на все кампании (10.09.2026 настройку убрали с
 * экрана). Прежние 400 брались из фактических данных портала (1064 первых
 * сообщения, медиана 260, 99% в 400, максимум 573), но на базе с ровными
 * текстами по 430–460 знаков такой порог останавливал рассылку целиком: ни
 * одно сообщение не уходило никогда. 600 оставляет запас над самым длинным
 * реально отправленным письмом и по-прежнему ловит мусор.
 *
 * Меняется переменной окружения TG_FIRST_TOUCH_MAX_CHARS.
 */

export const DEFAULT_MAX_MESSAGE_CHARS = Number(process.env.TG_FIRST_TOUCH_MAX_CHARS) || 600;
