 ...
 * при >=1 сигнале. Поверх — скоринговые сигналы (legal/accounting/consulting/
 * medicine): гоняются для ВСЕХ сегментов ради архива и сетки, но в
 * signalsCount и порог signal_min_count НЕ входят — их использует скоринг
 * сегментов с профилем (scoring.ts).
 *
...
export interface OutreachSignalSet {
  ...
  /** Скоринговый сигнал (medicine): частная клиника / медцентр / сеть клиник. */
  medicineRelevance: SignalVerdict;
  /** Скоринговый сигнал (medicine): акции, посадочные, спецпредложения, реклама. */
  medicinePromo: SignalVerdict;
  /** Скоринговый сигнал (medicine): имплантация, хирургия, диагностика, косметология, стоматология. */
  medicinePremium: SignalVerdict;
  /** Скоринговый сигнал (medicine): вакансии маркетолога / performance / агентство. */
  medicineMarketingTeam: SignalVerdict;
}

/**
 * Ключи исходных 6 «потоковых» сигналов. signalsCount считается ТОЛЬКО по ним:
 * скоринговые сигналы гоняются для всех сегментов ради архива и сетки, но НЕ
 * должны менять поведение фильтра signal_min_count сегментов без скоринг-профиля
 * (edu/remont и т.д.).
 */
...
export interface OutreachSignalsResult {
  ...
  /**
   * Жёсткий стоп-лист медицины: госбольница, фармкомпания, продажа/производство
   * медоборудования, частная практика / кабинет врача. Не входит в SIGNAL_COLUMNS
   * и signalsCount. Раннер обнуляет грейд сегмента medicine при hit=true.
   * Голое «аптека» здесь НЕ бан (клиника может держать аптеку при центре).
   */
  medicineHardReject?: SignalVerdict;
}
...
export const SIGNAL_COLUMNS: Array<{ key: keyof OutreachSignalSet; title: string; clarification: string }> = [
  ...
  { key: 'medicineRelevance', title: 'Частная клиника / медцентр / сеть', clarification: 'Частная клиника / медцентр / сеть — уточнение' },
  { key: 'medicinePromo', title: 'Акции / посадочные / спецпредложения', clarification: 'Акции / посадочные / спецпредложения — уточнение' },
  { key: 'medicinePremium', title: 'Имплантация / хирургия / диагностика', clarification: 'Имплантация / хирургия / диагностика — уточнение' },
  { key: 'medicineMarketingTeam', title: 'Маркетинговая команда / агентство', clarification: 'Маркетинговая команда / агентство — уточнение' },
];
...
const FORM_CONTEXT_KEYWORDS_RE =
  /заявк|обратн(ый|ого)\s+звон|перезвон|свяжитесь|оставьте|заказ(ать|уйте)?\s+звонок|написать\s+нам|запис[ьаы]|онлайн[-\s]?запис|заявк[ауи]\s+на\s+консультаци/i;

// S2: CTA на кнопках/ссылках, открывающих лид-форму или попап обратного звонка.
const CTA_KEYWORDS_RE =
  /заказ(ать|уйте)?\s+звонок|обратн(ый|ого)\s+звон(ок|ка)|остав(ить|ьте)\s+заявк|перезвон(ите|им)\s+мне|написать\s+нам|свяжитесь\s+с\s+нами|запис(аться|ать|итесь)?\s+на\s+(?:при[её]м|консультаци)|онлайн[-\s]?запис/i;
...
// ─── Скоринговые сигналы medicine (ТЗ 22.08.2026) ────────────────────────────
//
// Как и остальные скоринговые, гоняются для ВСЕХ сегментов ради архива.
// Стоп-лист вырезается ДО матчинга релевантности: иначе ГБУЗ «клиническая
// больница» и поставщики томографов проходили бы как частные клиники.

const MEDICINE_RELEVANCE_RES: RegExp[] = [
  /частн(?:ая|ой|ые)\s+(?:клиник|стоматолог|медицинск)/i,
  /медицинск(?:ий|ого|ие|ая)\s+центр/i,
  /сеть\s+клиник/i,
  /многопрофильн[а-яё]*\s+(?:медицинск|клиник)/i,
  /клиника\s+(?:пластическ|стоматолог|эстетическ)/i,
  /частн(?:ая|ой)\s+стоматолог/i,
];

// Стоп-фразы медицины. HARD_REJECT — никогда не лить в сегмент medicine
// (даже если акции+форма набрали C/A). NEGATIVE шире на «аптека»/«фармацевт»:
// ими только вырезаем релевантность, чтобы аптечная сеть не считалась клиникой,
// но клиника со своей аптекой не банится.
const MEDICINE_HARD_REJECT_PARTS = [
  'гбуз|мбуз|фгбуз|фгку',
  'государственн[а-яё]*\\s+(?:больниц|поликлиник|медицинск)',
  'городск[а-яё]*\\s+(?:клиническ[а-яё]*\\s+)?(?:больниц|поликлиник)',
  'казённ',
  'бюджетн[а-яё]*\\s+учрежден',
  'фармацевтическ[а-яё]*\\s+компани',
  'фармкомпани',
  'производител[ья]\\s+лекарств',
  'продаж[а-яё]*\\s+медицинск[а-яё]*\\s+оборудован',
  'медицинск[а-яё]*\\s+оборудован[а-яё]*\\s+(?:куп|продаж|постав|опт)',
  'производител[ья]\\s+медицинск',
  'частн[а-яё]*\\s+практик',
  'кабинет\\s+врача',
];

const MEDICINE_HARD_REJECT_RES: RegExp[] = MEDICINE_HARD_REJECT_PARTS.map((p) => new RegExp(p, 'i'));

export const MEDICINE_NEGATIVE_RE = new RegExp(
  [...MEDICINE_HARD_REJECT_PARTS, 'аптек[аи]', 'фармацевт'].join('|'),
  'gi',
);

const MEDICINE_PROMO_RES: RegExp[] = [
  /спецпредложен/i,
  /посадочн(?:ая|ой)\s+страниц/i,
  /для\s+рекламы/i,
  /рекламн(?:ое|ые|ый)\s+объявлен/i,
  /лендинг/i,
  /(^|[^а-яё])акци(?:я|и|ях|ями|ей)(?![а-яё])/i,
];

const MEDICINE_PREMIUM_RES: RegExp[] = [
  /имплантац/i,
  /хирург/i,
  /диагностик/i,
  /программ[аы]\s+лечен/i,
  /косметолог/i,
  /стоматолог/i,
];

const MEDICINE_MARKETING_TEAM_RES: RegExp[] = [
  /ваканси[яи].{0,80}маркетолог/i,
  /маркетолог.{0,40}ваканси/i,
  /ищем\s+маркетолог/i,
  /performance[-\s]?(?:специалист|менеджер)/i,
  /руководитель[а-яё]*\s+(?:отдела\s+)?маркетинг/i,
  /директор[а-яё]*\s+по\s+маркетинг/i,
  /маркетингов(?:ое|ого)\s+агентств/i,
  /digital[-\s]?агентств/i,
];
...
const CRM_CALLTRACKING_RES: RegExp[] = [
  ...
  /медицинск(?:ая|ой)\s+информационн(?:ая|ой)\s+систем/i,
  /онлайн[-\s]?запис/i,
  /(^|[^а-яё])мис(?![а-яё])/i,
];
...
export function emptySignals(): OutreachSignalSet {
  ...
  return {
    ...
    medicineRelevance: empty(),
    medicinePromo: empty(),
    medicinePremium: empty(),
    medicineMarketingTeam: empty(),
  };
}
...
function detectMedicineRelevance(pages: FetchedPage[]): SignalVerdict {
  for (const page of pages) {
    const cleaned = page.text.replace(MEDICINE_NEGATIVE_RE, ' ');
    const hit = findInText(cleaned, MEDICINE_RELEVANCE_RES);
    if (hit) return { hit: true, evidence: snippetAround(cleaned, hit.index, hit.length) };
  }
  return { hit: false, evidence: '' };
}

function detectMedicinePromo(pages: FetchedPage[]): SignalVerdict {
  return detectTextSignal(pages, MEDICINE_PROMO_RES);
}

function detectMedicinePremium(pages: FetchedPage[]): SignalVerdict {
  return detectTextSignal(pages, MEDICINE_PREMIUM_RES);
}

function detectMedicineMarketingTeam(pages: FetchedPage[]): SignalVerdict {
  return detectTextSignal(pages, MEDICINE_MARKETING_TEAM_RES);
}

function detectMedicineHardReject(pages: FetchedPage[]): SignalVerdict {
  return detectTextSignal(pages, MEDICINE_HARD_REJECT_RES);
}
...
const LLM_SYSTEM_PROMPT = `Ты — детектор признаков входящего потока заявок на сайте компании (B2B, РФ). По тексту страниц сайта определи наличие каждого из 16 сигналов. Извлекай ТОЛЬКО то, что явно указано. Не додумывай.
...
Верни JSON (и только JSON, без markdown):
{
  ...
  "medicineRelevance": {"hit": true|false, "evidence": "..."},
  "medicinePromo": {"hit": true|false, "evidence": "..."},
  "medicinePremium": {"hit": true|false, "evidence": "..."},
  "medicineMarketingTeam": {"hit": true|false, "evidence": "..."}
}
...
- contactForm: форма заявки/обратной связи, кнопка «Оставить заявку»/«Заказать звонок»/«Записаться на приём», онлайн-чат с оператором, онлайн-запись.
...
- crmCalltracking: на сайте явно используются CRM, коллтрекинг, речевая аналитика или медицинская информационная система: Calltouch, CoMagic, Callibri, Mango Office, UIS, amoCRM, Битрикс24, МИС, упоминания «коллтрекинг», «речевая аналитика», «виртуальная АТС», «онлайн-запись».
...
- medicineRelevance: сайт частной клиники, медицинского центра или сети клиник. НЕ сигнал: ГБУЗ/МБУЗ, государственная/городская больница или поликлиника, казённое/бюджетное учреждение, аптека, фармпроизводитель, продажа медицинского оборудования, частная практика и кабинет врача без клиники.
- medicinePromo: акции, спецпредложения, посадочные страницы, лендинги, рекламные объявления.
- medicinePremium: имплантация, хирургия, диагностика, программы лечения, косметология, стоматология.
- medicineMarketingTeam: вакансия маркетолога / performance-специалиста, руководитель отдела маркетинга, директор по маркетингу, маркетинговое или digital-агентство на сайте клиники.
...
export async function detectOutreachSignals(
  ...
  const signals: OutreachSignalSet = {
    ...
    medicineRelevance: detectMedicineRelevance(pages),
    medicinePromo: detectMedicinePromo(pages),
    medicinePremium: detectMedicinePremium(pages),
    medicineMarketingTeam: detectMedicineMarketingTeam(pages),
  };
