...
async function smtpVerifyViaProxy(
  ...
  for (const baseUrl of proxyTryOrder(mxHost)) {
    ...
    try {
      ...
      //
      // Побочный эффект переноса, о котором стоит знать: снятие таймера уехало
      // ПОСЛЕ разбора ответа, а не сразу после fetch. Значит SMTP_PROXY_TIMEOUT_MS
      // теперь накрывает и чтение тела ответа (res.json()), а не только сам
      // запрос. Так строго безопаснее — зависшее чтение тоже оборвётся, — но
      // отмена, случившаяся во время разбора ответа, объясняется именно здесь.
      clearTimeout(timer);
