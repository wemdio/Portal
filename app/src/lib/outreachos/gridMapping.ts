...
/**
 * Максимальная доля catch-all адресов в ИТОГОВОЙ пачке лидов (0.2 = 20%).
 *
 * catch_all — сервер принимает любой RCPT, существование ящика не проверяемо:
 * часть адресов реальна, часть отскочит баунсом и бьёт по репутации доменов
 * (по опыту Coldy-анализа catch-all даёт основную массу баунсов). На замере
 * 1500 catch-all было ~51% — без лимита половина рассылки летит вслепую.
 *
 * Лимит применяется ТОЛЬКО здесь (наш пайплайн, gridToLeadPayloads) — конструктор
 * баз для специалистов/клиентов не меняется, он по-прежнему отдаёт все статусы.
 * Берутся первые N catch-all по порядку сетки (без приоритета). Если ok-лидов
 * ноль, catch-all не берём вовсе (0.2×0=0) — математика доли, осознанно.
 */
export const CATCH_ALL_MAX_SHARE = 0.2;

/** true, если значение колонки «Email Статус» — catch-all (catch_all/catch-all). */
function isCatchAllStatus(status: string): boolean {
  return status.trim().toLowerCase().includes('catch');
}
...
export function gridToLeadPayloads(
  ...
  // заголовок на «статус»). -1 = шаг валидации не гонялся → лимит catch-all
  // применить не к чему, все лиды считаются ok.
  ...
  for (let r = 1; r < grid.length; r++) {
    ...
    leads.push(lead);
    leadIsCatchAll.push(statusIdx >= 0 && isCatchAllStatus(row[statusIdx] ?? ''));
  }

  // Лимит доли catch-all: в итоговой пачке их не больше CATCH_ALL_MAX_SHARE.
  // x / (ok + x) <= share  ⇔  x <= ok * share / (1 - share); при 20% → ok / 4.
  const okCount = leadIsCatchAll.filter((c) => !c).length;
  const maxCatchAll = Math.floor((okCount * CATCH_ALL_MAX_SHARE) / (1 - CATCH_ALL_MAX_SHARE));
  let catchAllTaken = 0;
  const capped: LeadCreatePayload[] = [];
  for (let i = 0; i < leads.length; i++) {
    if (leadIsCatchAll[i]) {
      if (catchAllTaken >= maxCatchAll) continue;
      catchAllTaken++;
    }
    capped.push(leads[i]);
  }
