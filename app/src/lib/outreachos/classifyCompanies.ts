...
const NOISE_CATEGORIES = new Set(['B2C', 'IP', 'GOV', 'OFF_ICP']);
...
- OFF_ICP — B2B, но НЕ наш покупатель (не ведут холодные email-продажи): частная охрана/ЧОП, ЖКХ и коммунальные службы (управляющие компании ЖКХ, водоканалы, теплосети), госмонополии и крупные госпредприятия (ФГУП/ГУП, РЖД, Росатом и т.п.), чисто локальные услуги без масштабируемых B2B-продаж. ВАЖНО НЕ путать: IT/софт/кибербезопасность(ИБ)/интеграторы/агентства/консалтинг/промышленность — это B2B (наш покупатель), НЕ OFF_ICP.
- UNCLEAR — по названию и домену определить нельзя.

Знание известных брендов используй. ВАЖНО: при сомнении между B2B и B2C ставь UNCLEAR — ложно выкинутый B2B дороже пропущенного B2C. При сомнении OFF_ICP vs B2B — ставь B2B (не режь целевых).

Ответ — СТРОГО JSON без пояснений: {"verdicts":[{"i":<номер из списка>,"c":"<категория>"}]} — по одному вердикту на каждый номер.`;

const REFUTE_PROMPT = `Ты адвокат дьявола. Компании ниже помечены на УДАЛЕНИЕ из базы (не наш покупатель: B2C/ИП/гос/не-наш-ICP). Продукт — платформа холодных email-рассылок; наш покупатель — масштабируемая компания, которая продаёт ДРУГИМ БИЗНЕСАМ и могла бы вести холодные email-продажи.

Для КАЖДОЙ попробуй ОПРОВЕРГНУТЬ: это на самом деле НАШ покупатель? Считается «да»: заметное B2B/корп-направление (Skyeng → Skyeng B2B), опт/white-label (издательство продаёт тиражи сетям), реклама бизнесу (медиа/радио живут продажей рекламы), IT/софт/кибербезопасность(ИБ)/интегратор/агентство/консалтинг. НЕ считается нашим покупателем (оставляй noise=true): частная охрана/ЧОП, ЖКХ/коммуналка, госмонополия/госпредприятие, чисто локальные услуги, «бизнес тоже может купить» без признаков.

noise=false ТОЛЬКО если это явно наш покупатель (масштабируемый B2B-продавец). Сомнение — подтверждай удаление (noise=true).
