 ...
 * домену. Поэтому раньше тред и ответ были закрыты, а оператору предлагали
 * отвечать из почтового клиента — совет невыполнимый: отправляющие ящики наши,
 * доступа к ним у клиента нет.
 *
 ...
 *
 * Хелпер даёт право и на ЧТЕНИЕ треда (/thread), и на ОТВЕТ (/reply).
 *
 * Про ответ важно знать вот что: провайдер отвергает reply/forward по письму вне
 * кампании — `400 … is not part of a campaign … (missing campaign_id or
 * list_id)`. Проверяется САМО письмо, поэтому передать campaign_id в запросе
 * бесполезно. Поэтому /reply на этом отказе уходит на фолбэк: НОВОЕ письмо тем
 * же ящиком через sendTestEmail — тот же обход, что с 27.07.2026 работает в
 * передаче лида (handoffSender). Предикат отказа общий —
 * lib/instantly/notPartOfCampaign.ts.
 *
 * История, чтобы не повторять: 16.08.2026 мы вывели «принадлежность к кампании
 * для отправки не нужна» из сигнатуры replyToEmail (там правда только
 * reply_to_uuid + eaccount), сняли свою проверку в /reply и открыли кнопку, НЕ
 * проверив отправку живьём. 18.08 специалист нажал «Ответить» и получил тот
 * самый 400. Ограничение при этом было известно и записано в шапке sendTestEmail
 * ещё с 27.07 — цена того, что поверили сигнатуре вместо поиска по коду.
 *
...
/**
 * Разрешает действие над сиротой (тред и ответ) или отказывает.
