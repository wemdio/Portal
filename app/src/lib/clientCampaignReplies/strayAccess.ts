 ...
 *
 * ⚠ ТОЛЬКО НА ЧТЕНИЕ (/thread). ОТВЕТИТЬ на сироту через API НЕЛЬЗЯ — и это
 * ограничение провайдера, а не наше. POST /emails/reply отвечает:
 *   400 The email you are replying to is not part of a campaign, so you cannot
 *       reply to it (missing campaign_id or list_id)
 * Проверяется САМО письмо, поэтому передать campaign_id/list_id в запросе не
 * помогает, и сменить их у уже полученного письма нельзя.
 *
 ...
 * reply_to_uuid + eaccount) и сняли свою проверку в /reply, не проверив отправку
 * живьём. 18.08 специалист нажал «Ответить» и получил тот самый 400 — тупик
 * вместо ответа. Гейт в /reply возвращён; этот хелпер остался для /thread, где
 * он реально работает и даёт видеть всю переписку с лидом.
 *
 * Если когда-нибудь понадобится всё-таки отправлять: единственный найденный в v2
 * путь послать НОВОЕ письмо — POST /emails/test (см. instantly/client.ts), но это
 * не ответ в треде, и прежде чем строить на нём UI, нужна боевая проба.
 *
...
/**
 * Разрешает ЧТЕНИЕ треда сироты или отказывает (на отправку — см. шапку).
