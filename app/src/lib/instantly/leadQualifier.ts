...
function isGenericDiscussionReadiness(authoredReply: string): boolean {
  ...
  return /^(?:(?:я|мы)\s+)?(?:готов(?:а|ы)?\s+(?:с\s+вами\s+)?(?:пообщаться|поговорить|обсудить)|давайте\s+(?:пообщаемся|поговорим|обсудим))(?:\s+с\s+вами)?(?:\s+(?:(?:ваше|это|данное)\s+предложение|(?:этот|данный)\s+вопрос|(?:по\s+)?(?:вопросам?\s+)?(?:по\s+)?(?:вашему\s+)?письму(?:\s+ниже)?|подробнее|детали|тему))?(?:\s+в\s+(?:любое|удобное(?:\s+для\s+(?:вас|меня|нас))?)\s+время)?[.!]?$/iu.test(statement);
}
...
Сомнение или мягкий отказ сами по себе («дорого», «у нас уже есть подрядчик», «не сейчас» без просьбы вернуться позже) не являются лидом. Но не отвергай ответ только из-за таких слов: отдельный запрос цены/КП, согласие на звонок или положительное условие критериев проекта оценивай по правилам ниже. Отдельной категории для работы с возражениями нет.
