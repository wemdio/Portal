...
export interface QualificationResult {
  ...
  /** Semantic direction of a human reply, not a sender-address blacklist. */
  nonLeadKind?: NonLeadKind | null;
  /**
  ...
  needsReview: boolean;
  /** Legacy storage compatibility only. No objection workflow or draft generation. */
...
const MATERIAL_REQUEST_ACTION_SOURCE = String.raw`(?:(?:пришлите|вышлите|отправьте|направьте|скиньте|предоставьте|оставьте)|(?:можно|хотел(?:и|а)?\s+бы)\s+(?:получить|посмотреть)|${MODAL_MATERIAL_SEND_SOURCE}|(?:send|share|forward|provide))${LETTER_TOKEN_END_SOURCE}`;
...
const SUBSTANTIVE_OFFER_SIGNAL_PATTERNS = [
  ...
  /(?:направить|прислать|отправить|передать)[^.!?\n]{0,35}информаци[а-яё]*\s+(?:по|о)\s+(?:систем[а-яё]*|платформ[а-яё]*)\s+(?:для\s+)?(?:управлени[яю]|уч[её]та|контроля|анализа|автоматизации)\s+[а-яё]/i,
...
export function extractAuthoredReplyText(text: string): string {
  ...
  const boundaryIndex = lines.findIndex((line, index) => {
    ...
    return (
      ...
      // Mail.ru can put To/Subject/date BEFORE the first quoted line. Exclude
      // that envelope too, but never treat an arbitrary divider as a quote.
      (/^-{5,}$/.test(trimmed) && (() => {
        const header = lines.slice(index + 1, index + 6).map((part) => part.trim());
        return /^(?:Кому|To):\s*.+@/iu.test(header[0] ?? '') &&
          /^(?:Тема|Subject):\s*\S/iu.test(header[1] ?? '') &&
          /^\d{1,2}\.\d{1,2}\.\d{4},[^\n]*<[^<>\s]+@[^<>\s]+>:\s*$/u.test(header[2] ?? '') &&
          header[3]?.startsWith('>') === true;
      })()) ||
...
type NonLeadKind = 'seller_pitch' | 'service_followup' | 'contact_routing' | 'nonliteral_interest' | 'intro_request';

/** These decisions interpret the apparent CTA itself. Finding the same words
 * (KP, price, call) again cannot disprove them. Uncertainty still gets the one
 * existing automatic adjudication, never a manual or unbounded retry queue. */
function isProtectedSemanticIntent(
  kind: NonLeadKind | null | undefined,
): kind is 'nonliteral_interest' | 'intro_request' {
  return kind === 'nonliteral_interest' || kind === 'intro_request';
}

function semanticNonLead(result: QualificationResult, kind: NonLeadKind): QualificationResult {
  const reasons: Record<NonLeadKind, string> = {
    ...
    nonliteral_interest: result.reason || 'Кажущийся запрос выражает насмешку, а не подтверждённый покупательский интерес.',
    intro_request: result.reason || 'Получатель ещё не ознакомился с предложением и лишь просит первичную информацию или разъяснение.',
  };
  return {
    ...result,
    nonLeadKind: kind,
    ...
    interestSignals: [],
    reason: reasons[kind],
    needsReview: isProtectedSemanticIntent(kind) && result.needsReview,
    objectionHandleable: false,
    objectionDraft: null,
  };
}

// Only an explicit self-sale construction is a deterministic fallback. General
// descriptions of the recipient's business, support addresses and signatures
...
function isExplicitSellerOnlyReply(text: string): boolean {
  ...
  const genericCompanyPitch = /(?:готовы|будем\s+рады)[^.!?\n]{0,60}сотрудничеств/iu.test(authored) &&
    /по\s+всем\s+вопросам/iu.test(authored);
  const buyerSignal = hasExplicitBuyerInterest(authored) ||
    hasRequestedFollowupMaterials(authored) ||
    (!offersToFulfilOurOrder && hasDirectActionableCta(authored));
  return ((sellerDescription && genericCompanyPitch) || offersToFulfilOurOrder) && !buyerSignal;
}

// This is used ONLY for a project that explicitly excludes contact-only
// redirection. Under the default/ADK policy a personal messenger CTA is valid.
function isOnlyContactRedirect(text: string): boolean {
  const authored = extractAuthoredReplyText(text);
...
function isThirdPartyContactTarget(text: string): boolean {
  const target = stripContactArtifacts(text).text
    .split(/\s+(?:в|по|на|через|via|at|on)\s+/iu, 1)[0].trim();
  if (/^(?:мной|нами|вами|мне|нам|вам|ваш[а-яё]*|me|us|you|your)(?:\s|$)/iu.test(target)) return false;
  return isStructurallyPlausibleRoutedContactName(target) ||
    /^(?:ней|ним|ними|him|her|them)(?:\s|$)/iu.test(target) ||
    new RegExp(`^${DEFERRED_THIRD_PARTY_TARGET_SOURCE}${LETTER_TOKEN_END_SOURCE}`, 'iu').test(target);
}

function hasThirdPartyContactRouting(text: string): boolean {
  const targets = text.matchAll(/(?:связаться|свяжитесь|обратитесь|обращайтесь)\s+(?:с|к)\s+([^.!?;,\n]{1,80})|(?:напишите|пишите|contact|call|write\s+to)\s+([^.!?;,\n]{1,80})/giu);
  if (Array.from(targets).some((match) => isThirdPartyContactTarget(match[1] ?? match[2]))) return true;
  const preposedTargets = text.matchAll(/(?:^|\s)(?:с|к)\s+([^.!?;,\n]{1,65}?)\s+(?:(?:можно|можете)\s+)?(?:связаться|свяжитесь|обратиться|обратитесь)(?=\s|$)/giu);
    ...
    (requireConcreteAction ? CONCRETE_ACTIONABLE_CTA_PATTERNS : DIRECT_ACTIONABLE_CTA_PATTERNS).some((pattern) =>
      findDirectActionableCtaMatches(statement, pattern).some(
        (match) => {
          ...
          const suffix = statement.slice((match.index ?? 0) + match[0].length).split(/[.!?;,:]/, 1)[0];
          return !isDirectActionableCtaMatchNegated(statement, match) &&
            !hasThirdPartyContactRouting(`${prefix}${match[0]}${suffix}`);
        },
      ),
    )
  );
...
/** Remove only greeting/name/role framing, not arbitrary introductory prose.
 * Keep any extra buyer request so the whole-statement guards cannot swallow it. */
function normalizeDiscussionReadiness(authoredReply: string): string {
  const stripAddressee = (text: string): string => text.replace(
    /^([\p{L}-]+(?:\s+[\p{L}-]+){0,2})[!,.]\s+/u,
    (prefix, name: string) => isLikelyContactName(name) ? '' : prefix,
  );
  const responsibility = String.raw`(?:(?:привлечени(?:ем|е)\s+(?:новых\s+)?клиентов|продаж(?:ами|и)|маркетинг(?:ом)?|развити(?:ем|е)(?:\s+продаж)?)(?:\s+и\s+)?){1,3}`;
  return stripAddressee(normalizeAuthoredStatement(stripAddressee(authoredReply.trim())))
    .replace(/^меня\s+зовут\s+[\p{L}-]+(?:\s+[\p{L}-]+){0,2}\s*[,!.;]\s*/iu, '')
    .replace(new RegExp(String.raw`^(?:я\s+(?:занимаюсь|отвечаю\s+за)\s+${responsibility}|(?:за\s+)?${responsibility}\s+(?:занимаюсь|отвечаю)\s+я)\s*[.!;,]\s*`, 'iu'), '')
    .replace(/^я\s+(?:региональный\s+)?(?:представитель|менеджер)(?:\s+в\s+(?:г\.?\s*)?[\p{L}-]+(?:\s+[\p{L}-]+){0,2})?\s*[.!;,]\s*/iu, '')
    .replace(/[.!;,]?\s*(?:спасибо|благодарю)[.!]?$/iu, '').trim();
}

/** A responsible person agreeing to hear the topic is not yet accepting our
 * service. Match the entire authored statement; extra commercial content stays
 ...
 * Technical questions about a named solution, or "let's discuss what you can
 * offer" with an appointment, must remain with semantic assessment. */
function isInitialOfferClarification(authoredReply: string): boolean {
  const statement = normalizeAuthoredStatement(authoredReply);
  if (hasActionableDirectCommercialRequest(statement) || hasExplicitBuyerAction(statement)) return false;
  return [
    /чтобы\s+(?:понять|выяснить|разобраться)[^.!?]{0,50}(?:в\s+ч[её]м\s+суть|о\s+ч[её]м\s+речь|что\s+(?:именно\s+)?вы\s+предлагаете)/iu,
    /(?:напишите|расскажите|объясните|уточните)[,\s]+(?:пожалуйста[,\s]+)?(?:что\s+(?:именно\s+|конкретно\s+)?(?:вы\s+)?(?:готовы\s+|можете\s+)?предложи(?:ть|те)|что\s+(?:именно\s+|конкретно\s+)?вы\s+предлагаете|в\s+ч[её]м\s+суть\s+(?:вашего\s+)?предложения)/iu,
  ].some((pattern) => pattern.test(statement));
}

/** A stand-alone current payment channel is not an order through that channel.
 * Consume the entire answer: extra questions/acceptance stay with the model. */
function isExistingPaymentChannelOnly(authoredReply: string): boolean {
  if (authoredReply.includes('?') || hasExplicitBuyerInterest(authoredReply) || hasDirectActionableCta(authoredReply)) return false;
  const match = /^(?:(?:у\s+нас|мы)\s+)?(?:оплата|оплачиваем|платим|работаем)\s+(?:ид[её]т\s+)?только\s+(?:через|с)\s+банк(?:ом)?\s+(.+)$/iu
    .exec(normalizeAuthoredStatement(authoredReply));
  // Conservative proper-name shape. Arbitrary lower-case prose after the bank
  // may be an unpunctuated acceptance; leave it to the model, not this veto.
  return Boolean(match && /^[«"]?[\p{Lu}\d][\p{L}\d-]*(?:\s+[\p{Lu}\d][\p{L}\d-]*){0,3}[»"]?$/u.test(match[1]));
}

/** Asking for OUR phone to clarify an opener is not a buyer accepting a call.
 * Require the generic purpose as well as a phone request, and consume it all:
 * a separate price/KP/demo request must never disappear into this shortcut. */
function isPhoneRequestToDiscussInquiry(authoredReply: string): boolean {
  const statement = normalizeDiscussionReadiness(authoredReply);
  const request = /^(?:подскажите|пришлите|напишите|дайте|сообщите)\s*,?\s+(?:пожалуйста[,\s]+)?(?:свой|ваш)\s+(?:номер(?:\s+телефона)?|телефон)\s*[,;.—–-]?\s*/iu;
  if (!request.test(statement)) return false;
  const purpose = statement.replace(request, '');
  return /^(?:чтобы\s+(?:(?:я|мы)\s+)?(?:(?:могла?|могли)\s+)?(?:связаться|созвониться|поговорить)\s+с\s+вами\s+и\s+обсудить|(?:для\s+обсуждения|обсудим))\s+(?:ваш(?:его)?\s+)?(?:запрос(?:а)?|обращени[ея]|вопрос(?:а)?|письм[оа])(?:\s+ниже)?[.!]?$/iu.test(purpose);
}

/** Whole generic request only. A product, quantity, budget or separate buying
 * request must stay with semantic assessment. Missing history is NOT an opener. */
function isGenericKpIntroductionRequest(authoredReply: string): boolean {
  const request = /^(?:пожалуйста[,\s]+)?(?:пришлите|отправьте|направьте|скиньте|(?:можете|можно)\s+(?:прислать|отправить|направить))\s+(?:пожалуйста[,\s]+)?(?:(?:ваше|сво[её])\s+)?(?:кп|коммерческое(?:\s+предложение)?)(?:\s+(?:сюда|мне|нам))?(?:\s+на\s+(?:эту\s+)?почту)?(?:[,\s]+пожалуйста)?[.!?]*$/iu;
  const statement = normalizeDiscussionReadiness(authoredReply);
  if (request.test(statement)) return true;
  // A preceding first-contact introduction does not make a generic KP request
  // specific. Consume only an independently recognized readiness/clarification.
  const lines = authoredReply.trim().split(/\n+/);
  return lines.length > 1 && request.test(normalizeAuthoredStatement(lines.at(-1) ?? '')) &&
    (isGenericDiscussionReadiness(lines.slice(0, -1).join('\n')) ||
      isPhoneRequestToDiscussInquiry(lines.slice(0, -1).join('\n')));
}

function isUnreadMessageCallback(authoredReply: string): boolean {
  const statement = normalizeAuthoredStatement(authoredReply);
  // Full statement, not a substring: an actual order after "missed your first
  // email" must not disappear. No number or signature is evidence of interest.
  return /^(?:возможно[,\s]+)?(?:мы|я)\s+не\s+(?:увидели?|прочитал[аи]?)\s+(?:(?:ваше|это)\s+)?(?:сообщение|письмо)\s*[.!]?\s*(?:(?:будьте\s+добры|пожалуйста)[,\s]+)?(?:перезвоните|позвоните|наберите)(?:\s+(?:мне|нам))?(?:\s+(?:по\s+(?:номеру|телефону)|на\s+номер))?\s*[:—–-]?\s*\+?[\d\s().-]{7,30}[.!]*$/iu.test(statement);
}
...
function protectedDefaultVerdict(
  ...
  if (isExistingPaymentChannelOnly(authoredReply)) {
    return {
      reason: 'Получатель лишь сообщил текущий способ оплаты; запроса нашего решения или согласия на сотрудничество нет.',
      needsReview: false,
    };
  }

  if (!confirmedProposal && !semanticShortOffer && isInitialOfferClarification(authoredReply)) {
    return {
      reason: 'Получатель просит сначала объяснить суть предложения; звонок для такого разъяснения не подтверждает покупательский интерес.',
      needsReview: false,
      proposalSeen: false,
    };
  }

  if (isUnreadMessageCallback(authoredReply)) {
    return {
      reason: 'Получатель сообщает, что не прочитал сообщение, и оставляет общий способ связи; собственного интереса к предложению нет.',
      needsReview: false,
      proposalSeen: false,
    };
  }

  if (!confirmedProposal && !semanticShortOffer && contactOpenerSeen &&
    isGenericKpIntroductionRequest(authoredReply)) {
    return {
      reason: 'После поиска ответственного без раскрытого оффера получатель лишь просит прислать КП для первичного ознакомления, а не выражает самостоятельную покупательскую потребность.',
      needsReview: false,
      proposalSeen: false,
    };
  }

  if (!confirmedProposal && !semanticShortOffer && (
    isGenericDiscussionReadiness(authoredReply) ||
    (contactOpenerSeen && isPhoneRequestToDiscussInquiry(authoredReply))
  )) {
    return {
      reason: 'Получатель согласился на первичное общение по теме обращения, но понятное предложение нашей услуги ещё не подтверждено; самостоятельного покупательского запроса нет.',
      needsReview: false,
      proposalSeen: false,
    };
  }
  ...
  if (
    ...
    !actionableCommercialRequest &&
    !actionableDirectCta &&
    !hasExplicitBuyerInterest(statement)
  ) {
    return {
      reason: 'Получатель перенаправил обращение к другому человеку или в отдел без собственного интереса.',
      needsReview: false,
    };
  }
...
function normalizeDefaultLeadSignals(
  ...
  const protectedVerdict = protectedDefaultVerdict(
    ...
    semanticShortOffer,
    [...outboundTexts, quotedText].some((text) =>
      RESPONSIBLE_RECIPIENT_REQUEST_PATTERNS.some((pattern) => pattern.test(text)),
    ),
  );
  if (protectedVerdict) {
    return {
  ...
  if (confirmedProposal && hasDirectPositiveInterest(authoredReply)) {
    ...
    // A call-pattern hit alone cannot reverse a completed semantic rejection.
    // Explicit clarification is vetoed above on either semantic pass.
    ((result.isLead || result.needsReview) && hasDirectActionableCta(
      authoredReply,
...
function buildSystemPrompt(
  ...
  const briefSection = briefText
    ? `\n\nКОНТЕКСТ ПРЕДЛОЖЕНИЯ (бриф клиента):\n---\n${briefText.slice(0, 2000)}\n---\nИспользуй бриф только для понимания нашего продукта или услуги. Он не доказывает, что адресат получил предложение.`
    : '';
...
- Сначала исключи полностью служебное/машинное уведомление и нелитеральный интерес (сарказм), затем сверь с определением проекта выше только основной ответ человека. Для человеческого ответа при противоречии со стандартными критериями ПРИОРИТЕТ у кастомного определения.
...
ЗАДАЧА: принять решение о передаче лида: is_lead=true или is_lead=false. Не составляй ответы клиенту.

ПЕРЕД ПОИСКОМ СЛОВ «КП», «ЦЕНА», «ЗВОНОК» ПРОВЕРЬ ИХ СМЫСЛ:
- Сарказм, насмешка, ироническое согласие и риторическая просьба — не реальный интерес, даже если буквально написано «очень интересно, пришлите КП с ценами». Оцени весь собственный ответ, а не отдельный CTA. Для такого кажущегося интереса ставь non_lead_kind="nonliteral_interest", is_lead=false, custom_criteria_matched=false. Кастомный критерий тоже требует реального, а не иронического выполнения условия.
- Признаки насмешки оценивай в сочетании: демонстративное несоответствие предлагаемого товара описанной роли адресата, преувеличенная необходимость/восторг, буквальное повторение нашего ассортимента вместо собственной потребности, риторический тон. Отрасль компании, отсутствие количества, слово «возможно», восклицания или смайлик ПО ОТДЕЛЬНОСТИ не означают сарказм. Компания может действительно расширять ассортимент. Если ирония только про цену/стиль письма, но есть отдельный настоящий заказ или предметный интерес, non_lead_kind=null: оцени его, не теряй лид.
- Если буквальное согласие и насмешка остаются одинаково правдоподобны, не выдавай автоматическое «лид» из-за запроса КП: non_lead_kind="nonliteral_interest", needs_review=true для одной дополнительной автоматической оценки. При повторной оценке без подтверждённого искреннего интереса — is_lead=false, needs_review=false. При уверенном сарказме дополнительная оценка не нужна.
- Если видим, что человеку отправили ТОЛЬКО поиск ответственного/общую тему, без раскрытой услуги, общая просьба «пришлите КП / своё коммерческое сюда на почту» означает первичное ознакомление, а не самодостаточную потребность: non_lead_kind="intro_request", is_lead=false, proposal_seen=false, needs_review=false. Не достраивай понимание оффера из слова «КП».
- Тот же запрет относится к «напишите, что конкретно вы готовы предложить для продвижения нашей компании и на каких условиях» после поиска контакта: сначала нужно раскрыть услугу. Слова «условия», «продвижение» и описание роли не превращают первичное разъяснение в запрос цены уже понятного решения. «Чтобы понять, в чём суть вопроса, давайте созвонимся» — тоже intro_request, а не коммерческий звонок. Для явного первичного разъяснения всегда заполняй non_lead_kind="intro_request", needs_review=false, а не только is_lead=false.
- Отличай этот случай от ОТСУТСТВУЮЩЕЙ истории: при ответе с другого адреса прошлый оффер мог не восстановиться. Тогда искренний запрос КП/цены сам по себе может быть лидом. Также учитывай реальный оффер из более раннего исходящего или пересланной цитаты. Если даже после пустого первого письма человек сам сформулировал конкретную потребность в нашей услуге/товаре, заказ, расчёт своей задачи — это самостоятельный покупательский запрос, non_lead_kind=null.
- «Не увидели / не прочитали ваше сообщение, перезвоните +номер» без отдельной потребности — восстановление связи, не интерес к офферу: non_lead_kind="intro_request", is_lead=false, needs_review=false. Если это полностью автоматический шаблон, используй machine_reply_kind. Не применяй запрет к «первое письмо пропустили, теперь изучили каталог, нужны 20 штук» или обычной просьбе позвонить после оффера.
- Эти смысловые ограничения приоритетнее стандартных позитивных признаков ниже. Для intro_request явный положительный кастомный критерий первичного контакта сохраняет приоритет; nonliteral_interest не может подтвердить намерение даже при кастомном критерии.

КАТЕГОРИИ:
1. КВАЛИФИЦИРОВАННЫЙ ЛИД — клиент выразил собственный положительный интерес к полученному офферу или готовность к коммерчески значимому следующему действию: звонку, встрече, демо, тесту, пилоту, покупке, заказу, обсуждению условий или другому конкретному CTA. Просьба вернуться к разговору позже (через месяц, летом, в конкретный будущий период) — лид только при подтверждённом содержательном оффере, когда человеку понятно, что ему предлагают. Также лидом является самостоятельный искренний коммерческий запрос (КП, цена, расчёт) с учётом смысловых исключений выше.
2. НЕ ЛИД — ответ без подтверждённого положительного условия применимых критериев: автоответ, отписка, отказ, простая передача контакта, общий запрос ознакомительной информации без коммерческого намерения или нейтральный ответ.

Сомнение или мягкий отказ сами по себе («дорого», «у нас уже есть подрядчик», «не сейчас» без собственного будущего интереса) не являются лидом. Но не отвергай ответ только из-за таких слов: отдельный запрос цены/КП, согласие на звонок или положительное условие критериев проекта оценивай по правилам ниже. Отдельной категории для работы с возражениями нет.
...
- Отличай разъяснение непонятной темы от конкретной деловой встречи по обозначенному предложению: на «хочу обсудить привлечение заказчиков на разработку и дозагрузку вашей команды» ответ «Давайте обсудим, что можете предложить. В пятницу в 09:00 мск удобно?» — лид, даже при пограничном раскрытии услуги (proposal_seen может остаться false). Есть собственная готовность обсуждать предложенную задачу и согласование конкретного времени. Простое «готова пообщаться в удобное время» или передача почты коллеги такого основания не дают. Время само по себе, график работы и звонок только для выяснения сути обращения — не доказательство интереса.
- Наличие нашего исходящего письма или развёрнутого предложения НЕ является обязательным для самостоятельного коммерческого запроса при отсутствующей истории: КП, цены, покупки, демо. Общая просьба о КП после известного нераскрытого обращения остаётся intro_request. Однозначное назначение встречи/звонка («Давайте завтра проведём встречу», «Можете меня набрать в 14:00») также может быть лидом без восстановленного исходящего. Но отсутствие истории НЕ равно известному письму-знакомству: если человек только согласился выяснить тему обращения или просит наш телефон, чтобы обсудить ещё не раскрытый запрос, это не коммерческий CTA. Применяй правило первичного знакомства ниже, не вырывай слово «позвонить» из его цели.
...
СНАЧАЛА ОПРЕДЕЛИ НАПРАВЛЕНИЕ ИНТЕРЕСА В ОСНОВНОМ ОТВЕТЕ:
...
- При non_lead_kind != null и отсутствии совпадения с положительным кастомным критерием ставь is_lead=false, needs_review=false; исключение — описанная выше неопределённость nonliteral_interest с одной автоматической оценкой. Для иных ответов non_lead_kind=null. Не выдумывай направление из подписи или цитаты.
...
- Названный продукт с конкретными функциями (например, система учёта заявок, ремонтов и запчастей) уже объясняет, что предлагается; обещание прислать информацию о таком продукте не превращает письмо в пустой поиск контакта. Ответ «направьте эту информацию мне» является запросом материалов; одно «я ответственный» без такого запроса — по-прежнему маршрутизация.
...
- Искренний запрос КП или коммерческого предложения, кроме общего ознакомительного запроса после известного пустого письма-знакомства (intro_request).
...
- Эти сигналы могут быть достаточны сами по себе, когда исходящее письмо не восстановилось в API. Но известный пустой opener и явное сообщение «не прочитали ваше письмо» — другая ситуация; сарказм также не является покупательским запросом.
...
- «С сервисами знакомы, пока не готовы технически к интеграции. В следующем году, думаю, будет актуально» после раскрытого оффера — собственный отложенный интерес, is_lead=true, needs_review=false. Дальний срок и отсутствие явного «напишите позже» не отменяют этот интерес. Отличай от одного «сейчас не готовы» без будущей актуальности и от даты появления другого сотрудника.
...
- «Можем пообщаться поподробнее?» и «хотела обсудить этот момент, можем онлайн созвон провести?» после одного поиска HR-директора без описания услуги — первичное знакомство, non_lead_kind="intro_request", is_lead=false, needs_review=false. Слово «созвон» само по себе недостаточно. Отдельный предметный интерес к обозначенной задаче с назначенным временем оценивай по правилу конкретной деловой встречи выше.
- БЕЗ подтверждённого оффера «пришлите предложение» без конкретной потребности — это лишь просьба ознакомиться. Добавление слова «коммерческое» не делает её лидом, когда видим только пустое письмо-знакомство; при отсутствующей истории запрос КП оценивается отдельно.
...
- Запрос только первичного РАЗЪЯСНЕНИЯ («что вы предлагаете?», «о чём речь?», «что это за решение?», «в чём суть?»), в том числе через звонок, означает, что человек ещё не понял оффер: ставь non_lead_kind="intro_request", is_lead=false, needs_review=false. Не применяй это к предметному вопросу об описанном решении или согласованию конкретной деловой встречи по правилу выше.
...
- Простое описание текущего порядка работы НЕ является таким вопросом: «Оплата только через банк ВТБ» после предложения платежного сервиса — is_lead=false, needs_review=false. Нельзя додумывать «готов купить, если работаете через ВТБ». Но «Сможете провести через наш банк?», «Готовы попробовать, если оплата через наш банк» или отдельный запрос цены — уже другие ответы: оцени их собственный интерес, не запрещай все упоминания банков/действующего подрядчика.
...
- Явная личная просьба позвонить или встретиться ("наберите меня завтра", "давайте созвонимся") может быть лидом даже без найденного исходящего письма. Не применяй это исключение к первичному выяснению ещё не раскрытого запроса (правило выше). Телефон в подписи или дежурное "звоните по любым вопросам" — не лид.
...
- При machine_reply_kind != null обязательно is_lead=false, custom_criteria_matched=false, needs_review=false. Контакты и призывы из служебного шаблона не могут выполнить кастомное правило «передали контакт — лид».
...
- Машинный/служебный ответ и nonliteral_interest всегда имеют приоритет над кастомным критерием; явные кастомные исключения остаются запретом. Не приписывай отправителю покупательский интерес из встречной продажи или чужой цитаты.
` : ''}

ФОРМАТ ОТВЕТА (только валидный JSON, без markdown):
{
  "machine_reply_kind": "auto_reply"/"delivery_failure"/"service_acknowledgement"/null,
  "non_lead_kind": "seller_pitch"/"service_followup"/"contact_routing"/"nonliteral_interest"/"intro_request"/null,
  "is_lead": true/false,
...
function validateAIResult(value: unknown): asserts value is AIQualificationPayload {
  ...
  if (
    ...
    fields.non_lead_kind !== 'nonliteral_interest' &&
    fields.non_lead_kind !== 'intro_request'
  ) {
...
function parseAIResult(content: string): QualificationResult {
  ...
  return {
    ...
    // Ignore retired fields even in an old cached/provider response.
    objectionHandleable: false,
    objectionDraft: null,
  };
}
...
function enforceCustomCriteriaPriority(
  ...
  // A nonliteral request cannot satisfy a project rule merely by containing
  // the literal keyword. Resolve a doubtful tone automatically first.
  if (result.nonLeadKind === 'nonliteral_interest') {
    return semanticNonLead(result, 'nonliteral_interest');
  }
...
function applyQualificationGuards(
  ...
  if (aiResult.nonLeadKind === 'nonliteral_interest') return aiResult;
