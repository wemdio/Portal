 ...
 * дефолтом колонки в миграции `20260730_0001_expenses_core.sql` и меняться
 * может только вместе с ней.
 */
export const DEFAULT_PAYER = 'ceo_personal_card';

/**
 * Человеческие подписи плательщиков.
 *
 * В базе `payer` — свободный текст без справочника и без CHECK, и витрина
 * `expenses_v` подставляет его в `counterparty`, то есть служебный ключ виден и
 * в разбивке, и в очереди. Поэтому карта именно карта, а не enum: ключ уходит в
 * данные как есть, подпись существует только на экране, а неизвестный ключ
 * показывается как есть — придумать ему название мы всё равно не можем.
 */
const PAYER_LABELS: Record<string, string> = {
  [DEFAULT_PAYER]: 'Личная карта CEO',
};

export function payerLabel(key: string): string {
  return PAYER_LABELS[key] ?? key;
}
