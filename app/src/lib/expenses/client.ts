...
/**
 * Клиентский слой дашборда расходов.
 *
 * Все запросы идут сюда и только сюда: страница расходов не ходит в Supabase
 * напрямую, потому что витрину `expenses_v` читает исключительно серверный код
 * под service_role (см. `lib/expenses/rows.ts`). Токен сессии подставляет
...
/** Текст ошибки от роута расходов. Роуты отвечают `{ error }` — показываем его как есть. */
async function errorText(res: Response): Promise<string> {
  const body = (await res.json().catch(() => null)) as { error?: string } | null;
  if (body?.error) return body.error;
  if (res.status === 401) return 'Сессия истекла — перезайди в портал';
  if (res.status === 403) return 'Доступ к расходам не выдан';
