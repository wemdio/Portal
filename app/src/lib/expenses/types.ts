 ...
 *
 * Живёт здесь, а не рядом с компонентом выбора: строку отдаёт роут
 * `/api/expenses/vendors/directory`, список собирает `ExpensesView`, читает его
 * очередь разметки и форма ручной траты, а группирует — чистая функция в
 * `vendorPicker.ts`. Тип, объявленный внутри `'use client'`-компонента,
 * втягивал бы за собой всю эту цепочку, да и серверу был бы недоступен.
 *
