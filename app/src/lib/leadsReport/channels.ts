 ...
 *
 * Записи нормализуются при конструировании (через `normalize`) — пиши их
 * ровно так, как значение выглядит в AMO, регистр и «ё» роли не играют.
 */
export const EXCLUDED_SOURCES: ReadonlySet<string> = new Set(
  ['портал (outreachOS)'].map(normalize),
);
