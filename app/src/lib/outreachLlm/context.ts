...
export class JobBudget {
  ...
  /**
   * includeReserved — брони идущих запросов считать потраченными. Так пишутся
   * снимки, пока запуск идёт: упадёт воркер — перезапуск (fromSnapshot) начнёт
   * с них, и оборванные падением запросы, за которые деньги могли уже
   * списать, в лимите остаются. И итог запуска, если ответа на них так и не
   * дождались. Итог с ответами — без броней: брони к тому времени сняты и
   * заменены фактом, ничего не считается дважды.
   */
