...
/**
 * Все инструменты витрины — для админки, где надо показать список галочек
 * при выдаче ключа. Наружу этот список не отдаётся: внешний ключ видит
 * только то, что открыто ему (см. `listBenchTools`).
 */
export function listAllBenchTools(): BenchTool[] {
  return [...TOOLS];
}
