 ...
 * гипотез (accept/reject на доске, ve_hypotheses.status).
 *
...
type VerticalRow = Pick<VeVertical, 'id' | 'name' | 'potential_pct' | 'rank'>;
type HypothesisRow = Pick<VeHypothesis, 'id' | 'vertical_id' | 'tier' | 'potential_pct' | 'status'>;
...
export async function recomputeProjectVerticalPcts(
  ...
  if (vError) throw new Error(`ve_verticals read: ${vError.message}`);
  ...
  for (let i = 0; i < scored.length; i++) {
    ...
    const { error } = await supabase
      ...
      .eq('id', s.id);
    if (error) throw new Error(`ve_verticals update: ${error.message}`);
  }
