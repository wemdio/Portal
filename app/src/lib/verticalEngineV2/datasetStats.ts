 ...
 * прежнее поведение.
 */
function calibrationMarketSkip(market: VeMarket | undefined, fn: string): boolean {
...
export interface VeDatasetStats {
...
export interface VeWinnerPattern {
  pattern: string;
  reply_pct: number;
  sent: number;
}

export interface VePortfolioEntry {
...
export async function getSegmentStats(
  verticalName: string,
  synonyms: string[],
  opts?: { market?: VeMarket },
): Promise<VeDatasetStats> {
  const stats: VeDatasetStats = {
    matched_segments: [],
    campaigns: 0,
    sent: 0,
    replies: 0,
 ...
 */
export async function getPortfolioProfile(opts?: { limit?: number; market?: VeMarket }): Promise<VePortfolioEntry[]> {
...
export interface VeCampaignActuals {
...
/**
 * Факт по кампаниям запуска (ve_templates.launch_info.campaigns). null —
 * датасет не сконфигурирован/упал или ни одна кампания ещё не синкнулась.
 * Never-throw по контракту модуля.
 */
export async function getCampaignActuals(campaignIds: string[]): Promise<VeCampaignActuals | null> {
