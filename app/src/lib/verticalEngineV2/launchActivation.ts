...
function activationOutcome(body: Record<string, unknown>, options: { status?: number } = {}): VeLaunchActivationOutcome {
...
type ReconciliationResult =
  ...
  | { ok: false; response: VeLaunchActivationOutcome };


/** Same guarded activation path for an explicit HTTP action and its durable worker continuation. */
export async function activateVeLaunchPortfolioItem(input: VeLaunchActivationInput): Promise<VeLaunchActivationOutcome> {
  const supabaseAdmin = input.portalDb;
  ...
  function jsonError(error: string, status: number, code: string) {
    return activationOutcome({ error, code }, { status });
  }
