...
- отличай справку о клиенте от результата выполненной работы: мощность предприятия, его оборот, место на рынке и результаты прежней модернизации не являются эффектом описанного внедрения. Не переноси их в result/metrics как достижения исполнителя без прямой связи в исходнике;
...
- никаких выдуманных цифр: metrics заполняй только показателями из ИСХОДНОГО ФРАГМЕНТА ЭТОГО кейса (если их нет — пустой объект). Значения — числа или строки с единицами, не вложенные объекты. Сроки и количества, записанные словами (например «несколько месяцев», «три недели», «через год»), сохраняй ДОСЛОВНО как строку. Явно указанный срок выполненной работы обязательно сохрани в metrics, даже если в нём нет ни одной цифры: «закончилось через год» → {"срок внедрения": "через год"}, а не «1 год», «один год» или пустой объект. Не превращай слова в цифры, не пересчитывай единицы, не вычисляй проценты и не добавляй значения «не указано»;
