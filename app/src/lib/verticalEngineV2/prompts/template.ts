...
- Каждое письмо в fixed_block специфицируй как живую записку от человека человеку: естественное приветствие в начале («Добрый день, {{firstName}}», «Доброго дня») → мягкий повод одним спокойным предложением → суть → один CTA-вопрос → подпись отправителя в конце (если в материалах есть блок «ПОДПИСЬ ОТПРАВИТЕЛЯ» — используй её дословно; иначе подписывайся командой компании из брифа, например «Команда <компания клиента>» — имя человека НЕ придумывай). Открыватели-утверждения в лоб («Ваш рынок X», «Вы продаёте Y», «Продажи упираются…») запрещай как первую строку письма. Тире («—», «–») в темах и телах писем запрещены: замена — запятая, двоеточие, точка или скобки.
...
- ПОВОД каждого письма специфицируй явно и бери его из АНАЛИЗА БАЗЫ: природа базы диктует повод (база вакансий → их найм; перекос в регионы → география компаний; отраслевой микс → их сегмент; сезонный пик отрасли → тайминг, напр. образование → сентябрь). Общее место про сегмент как повод запрещено («у всех в сегменте проблема X», «продажи упираются в потолок трафика»). У каждого письма цепочки — свой конкретный повод: новый угол письма = новый факт-повод, не «напоминаю о себе».
...
- Specify each email's REASON FOR WRITING explicitly and take it from the BASE ANALYSIS: the nature of the base dictates the reason (a job-postings base → their hiring; a skew toward regions → the companies' geography; an industry mix → their segment; an industry's seasonal peak → timing, e.g. education → September). A generic segment truism as the reason is banned ("everyone in the segment has problem X", "sales are hitting a traffic ceiling"). Each email of the sequence has its own concrete reason: a new email angle = a new fact-reason, not "reminding about myself".
...
- ПОВОД: каждое письмо открывается естественным приветствием («Добрый день, {{firstName}}», «Доброго дня»), за которым идёт конкретный повод из анализа базы — почему пишем именно этим получателям сейчас. Природа базы диктует повод: база вакансий → их найм; перекос в регионы → география компаний; отраслевой микс → их сегмент; сезонный пик отрасли → тайминг (образование → сентябрь). Общее место про сегмент как повод запрещено («у всех в сегменте проблема X», «продажи упираются в потолок трафика»). У каждого письма — свой повод: новый угол = новый факт-повод, не «напоминаю о себе».
