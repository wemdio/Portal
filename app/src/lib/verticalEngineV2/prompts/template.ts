...
- ПОВОД: каждое письмо открывается естественным приветствием («Здравствуйте, {{firstName}}», «Добрый день»), за которым идёт конкретный повод из анализа базы — почему пишем именно этим получателям сейчас. Природа базы диктует повод: база вакансий → их найм; перекос в регионы → география компаний; отраслевой микс → их сегмент; сезонный пик отрасли → тайминг (образование → сентябрь). Общее место про сегмент как повод запрещено («у всех в сегменте проблема X», «продажи упираются в потолок трафика»). У каждого письма — свой повод: новый угол = новый факт-повод, не «напоминаю о себе».
