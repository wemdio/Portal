/**
 * Промпт-архитектура стадии chain: вертикаль + бриф → цепочка из 4 писем.
 *
 ...
 * рерайт — locale-keyed (ru/en/pl) по языку цепочки (ve_chains.language;
 ...
 * разбирает цепочку → JSON-вердикт VeChainCritique через callLLMWithSchema) и
 ...
 * см. VePromptInjections; signatureOverride принимают генерация и рерайт
 * (критику он не нужен: критик не переписывает текст и не может «снять»
 * подпись).
 */

import type { LLMMessage } from '../llm';
import type { VeChainLanguage, VeEvidenceItem } from '../types';
...
- СТРУКТУРА ПИСЬМА: каждое письмо — записка от живого человека человеку, а не питч. Начинается с естественного приветствия («Добрый день, {{firstName}}», «Доброго дня»), заканчивается подписью отправителя (если в материалах есть блок «ПОДПИСЬ ОТПРАВИТЕЛЯ» — используй её дословно; иначе подписывайся командой компании из брифа, например «Команда <компания отправителя>» — имя человека НЕ придумывай). Оператор {{var}} в приветствии считается в лимит «ровно один {{var}} в теле»; приветствие и подпись в лимит слов тела не входят.
- ПРИВЕТСТВИЕ В RU: только «Добрый день» или «Доброго дня», имя добавляй только при наличии данных. «Здравствуйте» и «Здравствуй» запрещены, даже если встречаются в исходной цепочке или примере стиля. Для других языков используй естественное приветствие на языке письма.
- АБЗАЦЫ: во всех письмах и вариантах A/B разделяй приветствие / повод / суть / вопрос / подпись пустой строкой (два перевода строки). Приветствие и вопрос — каждый отдельным абзацем; вопрос непосредственно перед подписью. В коротком follow-up можно объединить повод и суть, но оставь минимум четыре абзаца. Одиночные переводы строк допустимы только внутри заданной подписи, её сохраняй дословно.
...
- PARAGRAPHS: in every email and A/B alternative, separate greeting / reason / offer / question / signature with a blank line (two newline characters). Greeting and question each have their own paragraph; the question comes immediately before the signature. A short follow-up may combine reason and offer, but keep at least four paragraphs. Keep single line breaks only inside the supplied signature, which stays verbatim.
...
/**
 * Регламент по языку цепочки (ve_chains.language). PL-перевода пока нет —
 * осознанно используется RU-вариант (то же для системных блоков ниже).
 */
export const CHAIN_REGULATIONS: Record<VeChainLanguage, string> = {
 ...
 */
export interface VePromptInjections {
...
export interface ChainPromptInput extends VePromptInjections {
...
const PRIMER_ACK: Record<VeChainLanguage, string> = {
...
const TASK_PROMPTS: Record<VeChainLanguage, string> = {
  ru: `Ты — senior email outreach специалист с опытом запуска 400+ холодных B2B-кампаний (средний reply rate 8–18%).

Напиши цепочку ровно из 4 писем для холодной рассылки по вертикали, описанной в материалах выше. Клиент описан в брифе/профиле сайта из материалов — продаём его продукт/услугу; письма идут лицам, принимающим решения, в целевой вертикали.
...
- Каждое письмо начинается с естественного приветствия («Добрый день, {{firstName}}», «Доброго дня») и заканчивается подписью отправителя (если в материалах есть блок «ПОДПИСЬ ОТПРАВИТЕЛЯ» — используй её дословно; иначе подписывайся командой компании из брифа, например «Команда <компания клиента>» — имя человека НЕ придумывай).
...
- Каждое письмо начинается с естественного приветствия («Добрый день, {{firstName}}», «Доброго дня») и заканчивается подписью отправителя; первая строка — приветствие, а не утверждение в лоб про бизнес или рынок получателя («Ваш рынок X», «Вы продаёте Y», «Продажи упираются…»).
...
Write a sequence of exactly 4 emails for a cold campaign targeting the vertical described in the materials above. The client is described in the brief/site profile in the materials — we sell their product/service; emails go to decision-makers in the target vertical.
...
const SYSTEM: Record<VeChainLanguage, string> = {
  ru: SYSTEM_RU,
...
export interface VeCriticIssue {
  letter_index: number;
  problem: string;
  fix: string;
}
...
export interface VeChainCritique {
  verdict: string;
  issues: VeCriticIssue[];
}
...
const CRITIC_SYSTEM: Record<VeChainLanguage, string> = {
  ru: CRITIC_SYSTEM_RU,
  en: CRITIC_SYSTEM_EN,
  pl: CRITIC_SYSTEM_RU,
};
...
const LANG_NAMES_RU: Record<VeChainLanguage, string> = {
  ru: 'русский',
  en: 'английский',
  pl: 'польский',
};

const LANG_NAMES_EN: Record<VeChainLanguage, string> = {
  ru: 'Russian',
  en: 'English',
  pl: 'Polish',
};
 ...
 * Ответ — JSON VeChainCritique (вызов через callLLMWithSchema): промпт
...
const REWRITE_SYSTEM: Record<VeChainLanguage, string> = {
  ru: REWRITE_SYSTEM_RU,
  en: REWRITE_SYSTEM_EN,
  pl: REWRITE_SYSTEM_RU,
};

const REWRITE_ACK: Record<VeChainLanguage, string> = {
...
const REWRITE_TASK: Record<VeChainLanguage, string> = {
