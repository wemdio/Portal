/**
 * Стадия dossier: вертикаль → объективное досье сегмента (ve_vertical_dossiers).
 *
 * Состав досье:
...
import { collectDossierCounters, type VeDossierCounters } from '../dossierData';
import { getSegmentStats, type VeDatasetStats } from '../datasetStats';
import { projectMarket } from '../market';
import { callLLMWithSchema, getVeModel, type LLMMessage } from '../llm';
import type { VeJob, VeJobTitle, VeVertical } from '../types';
import {
  addUsage,
  newUsage,
  payloadString,
 ...
 */
function audienceSideOf(t: VeJobTitle): string | undefined {
  const raw = (t as VeJobTitle & { audience_side?: unknown }).audience_side;
 ...
 * см. VeDossierData в components/vertical-engine-v2/api.ts).
 */
const VeDossierInterpretationLlmSchema = z.object({
...
function buildDossierMessages(input: {
  ...
  verticalName: string;
  synonyms: string[];
  counters: VeDossierCounters;
  datasetStats: VeDatasetStats;
  ...
  const system = [
    ...
    ' - market_summary: 2–3 предложения, цитируй счётчики (уникальных компаний',
    '   по ИНН, сырых строк директории и компаний с указанными (непроверенными)',
    '   email/телефонами,',
...
export async function runDossierStage(job: VeJob, ctx: VeStageContext): Promise<VeStageResult> {
  ...
  const { data: vocabRow, error: vocabError } = await ctx.supabase
    ...
    .maybeSingle();
  if (vocabError) {
    stageLog(ctx, `[dossier] ve_vocab read: ${vocabError.message} — продолжаем без должностей`);
    ...
    const titles = vocabRow.job_titles as VeJobTitle[];
    const audienceRank = (t: VeJobTitle): number => {
  ...
  let counters: VeDossierCounters | null = null;
  ...
  let datasetStats: VeDatasetStats | null = null;
  ...
  // Null-объекты вместо null: форма data всегда совпадает с VeDossierData,
  ...
  const countersSafe: VeDossierCounters = counters ?? {
  ...
  const datasetStatsSafe: VeDatasetStats = datasetStats ?? {
    matched_segments: [],
    campaigns: 0,
    sent: 0,
    replies: 0,
  ...
  if (!counters && !datasetStats) {
    ...
    if (upError) stageLog(ctx, `[dossier] ve_vertical_dossiers upsert (failed): ${upError.message}`);
    throw new Error(note);
  }

  const model = getVeModel('bulk');
  const llm = await callLLMWithSchema(
    ...
    VeDossierInterpretationLlmSchema,
  ...
  const { data: upserted, error: upError } = await ctx.supabase
    ...
    .select('id')
    .single();
  if (upError || !upserted) throw new Error(`ve_vertical_dossiers upsert: ${upError?.message ?? 'unknown'}`);
  ...
  return {
    result: {
      ...
      companies_unique_total: countersSafe.companies_unique_total ?? null,
      companies_with_email: countersSafe.companies_with_email ?? null,
