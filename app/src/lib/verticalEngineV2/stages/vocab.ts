...
export async function runVocabStage(job: VeJob, ctx: VeStageContext): Promise<VeStageResult> {
  ...
  const { data: inserted, error: insError } = await ctx.supabase
    ...
    .select('id')
    .single();
  if (insError || !inserted) throw new Error(`ve_vocab insert: ${insError?.message ?? 'unknown'}`);
