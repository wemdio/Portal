 ...
 * в ve_verticals + ve_hypotheses.vertical_id. Проект → status 'researched'.
 */

import { callLLMWithSchema, getVeModel } from '../llm';
import { VeClusteringSchema, type VeClusteringDecision } from '../schemas';
...
import type { VeHypothesis, VeJob } from '../types';
import {
  addUsage,
  newUsage,
  readProject,
...
export function applyClusteringDecisions(
  ...
  decisions: VeClusteringDecision[],
): AppliedVertical[] {
...
export async function runClusteringStage(job: VeJob, ctx: VeStageContext): Promise<VeStageResult> {
  const usage = newUsage();

  // Рынок: ctx.market (воркер), фолбэк — колонка ve_projects.market (читаем
  // проект только в фолбэке: остальное стадии нужно из ve_hypotheses).
  ...
  if (error) throw new Error(`ve_hypotheses read: ${error.message}`);
  const hypotheses = (hyps ?? []) as VeHypothesis[];
  ...
  const { error: unlinkError } = await ctx.supabase
    ...
    .eq('project_id', job.project_id);
  if (unlinkError) throw new Error(`ve_hypotheses unlink: ${unlinkError.message}`);
  const { error: delError } = await ctx.supabase
    .from('ve_verticals')
    .delete()
    .eq('project_id', job.project_id);
  if (delError) throw new Error(`ve_verticals cleanup: ${delError.message}`);
  ...
  for (const v of verticals) {
    const { data: inserted, error: insError } = await ctx.supabase
      ...
      .select('id')
      .single();
    if (insError || !inserted) throw new Error(`ve_verticals insert: ${insError?.message ?? 'unknown'}`);
    ...
    if (linkError) throw new Error(`ve_hypotheses link: ${linkError.message}`);
  }
