...
export interface VeLetterForCheck {
  subject: string | null;
  body: string;
...
export function checkLetterRules(
  letters: VeLetterForCheck[],
  language: string,
  facts: Set<string>,
): VeLetterRuleViolation[] {
  const violations: VeLetterRuleViolation[] = [];
  letters.forEach((letter, i) => {
    ...
    if (language === 'ru') {
      ...
      for (const re of CLICVE_WORDS_RU) {
