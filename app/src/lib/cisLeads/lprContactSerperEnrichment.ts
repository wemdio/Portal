...
ВАЖНО:
- Нужны ТОЛЬКО ЛИЧНЫЕ контакты этого конкретного человека.
- НЕ возвращай общий телефон приёмной или info@/sales@/office@ email компании.
- НИКОГДА не выдумывай и не угадывай email. Только email, который ты НАШЁЛ на реальной веб-странице.
- НЕ генерируй email по шаблону имя@домен. Если личный контакт не найден — верни null.
