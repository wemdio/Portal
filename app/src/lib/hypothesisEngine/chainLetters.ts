 ...
 *
 * Вынесено из PATCH api/tools/hypothesis-engine/chains/[id] — клиентский
...
export interface HeLetterVariantRow {
  subject: string | null;
  body: string;
}

/** Форма jsonb-письма he_chains.letters: основной вариант = «A», рядом variants. */
export interface HeChainLetterRow {
  subject: string | null;
  body: string;
  wait_days?: number;
  variants?: HeLetterVariantRow[];
  segment_variants?: unknown[];
}

export interface HeSegmentVariantRow {
  when: string;
  text: string;
}

export const HE_CHAIN_MAX_LETTERS = 6;
export const HE_CHAIN_MAX_BODY_LEN = 50_000;
export const HE_CHAIN_MAX_SUBJECT_LEN = 500;
export const HE_CHAIN_MAX_WAIT_DAYS = 90;

export function isHeLetterVariant(v: unknown): v is HeLetterVariantRow {
  return (
    typeof v === 'object' &&
    v !== null &&
    typeof (v as HeLetterVariantRow).body === 'string' &&
    ((v as HeLetterVariantRow).subject === null || typeof (v as HeLetterVariantRow).subject === 'string')
  );
}
...
function clampWaitDays(value: unknown): number {
  ...
  return Math.min(HE_CHAIN_MAX_WAIT_DAYS, Math.max(0, Math.trunc(n)));
}
...
function normalizeSubject(value: unknown): { ok: boolean; subject: string | null } {
  ...
  if (typeof value !== 'string' || value.length > HE_CHAIN_MAX_SUBJECT_LEN) {
...
function isValidBody(value: unknown): value is string {
  return typeof value === 'string' && value.trim() !== '' && value.length <= HE_CHAIN_MAX_BODY_LEN;
}

function normalizeVariant(v: unknown): HeLetterVariantRow | null {
...
function normalizeSegmentVariant(v: unknown): HeSegmentVariantRow | null {
  if (typeof v !== 'object' || v === null) return null;
 ...
 */
export function normalizeHeChainLetters(input: unknown[]): { letters?: HeChainLetterRow[]; error?: string } {
  if (input.length < 1 || input.length > HE_CHAIN_MAX_LETTERS) {
    return { error: `Писем в цепочке должно быть от 1 до ${HE_CHAIN_MAX_LETTERS}` };
  }
  const out: HeChainLetterRow[] = [];
  for (let i = 0; i < input.length; i += 1) {
    ...
    >;
    if (!isValidBody(body)) {
      return { error: `Тело письма ${i + 1} пустое или длиннее ${HE_CHAIN_MAX_BODY_LEN} символов` };
    }
    const s = normalizeSubject(subject);
    if (!s.ok) {
      return {
        error: `Тема письма ${i + 1} должна быть строкой до ${HE_CHAIN_MAX_SUBJECT_LEN} символов или null`,
      };
    }
    const letter: HeChainLetterRow = {
      subject: s.subject,
      body,
    ...
    if (segment_variants !== undefined) {
      ...
      const norm: HeSegmentVariantRow[] = [];
