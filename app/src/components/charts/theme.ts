...
/**
 * Фон берём не из переменной, а замеряем у ближайшего непрозрачного предка.
 *
 * Карточки портала покрашены утилитами Tailwind (`bg-white`), а тёмная тема
 * переопределяет их отдельным правилом в `globals.css`. Никакой переменной, из
 * которой можно было бы прочитать «текущую поверхность», не существует, и
 * захардкоженный белый оставил бы тултип светлым пятном в тёмной теме.
 */
