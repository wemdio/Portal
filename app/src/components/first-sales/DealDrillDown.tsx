 ...
 *
 * Тип общий с воронкой (`FunnelDealsList`), и там `lead` ещё несёт смысл —
...
export type InPeriod = {
  ...
  qualified: boolean;
  meetings: number;
  /** Сделка закрыта в плюс внутри периода — это и есть продажа. */
  sale: boolean;
  /** Этап «Согласование договора» пройден внутри периода. Промежуточная
   *  отметка: часть продаж этот этап не проходит вовсе. */
  contract: boolean;
  money: number;
};
...
function periodBadges(p: InPeriod): string[] {
  ...
  // бы у каждой строки и не отличал бы ничего. В воронке (`FunnelDealsList`)
