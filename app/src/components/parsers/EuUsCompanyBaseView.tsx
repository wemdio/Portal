...
const PREVIEW = 100;
const ZIP_THRESHOLD = 100_000; // above this, export as a ZIP split into 100k-row CSVs
...
function exportRow(r: PdlCompanyRow): string[] {
  return [r.name, r.website ?? '', industryLabelRu(r.industry), sizeLabelRu(r.size), countryLabelRu(r.country), r.locality ?? '', descriptionOf(r), 'pdl'];
}
...
function filterQuery(f: { industry: string[]; size: string[]; country: string[]; name: string }) {
...
export function EuUsCompanyBaseView() {
  ...
  const filters = useMemo(() => ({ industry, size, country, name }), [industry, size, country, name]);
  const filtersKey = useMemo(() => filterQuery(filters).toString(), [filters]);

  // Instant approximate counter with stale-request cancellation (no jumping).
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      void (async () => {
        try {
          const data = await apiFetch<CountResponse>(`/api/company-base/count?${filtersKey}`, { signal: ac.signal });
  ...
  const runPreview = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await apiFetch<SearchResponse>(`/api/company-base/search?${filterQuery(filters).toString()}&limit=${PREVIEW}`);
      setItems(data.items);
      setSearched(true);
    } catch (e) {
  ...
  const downloadFile = useCallback(async () => {
    ...
    try {
      ...
      window.location.href = `/api/company-base/export?${p.toString()}`;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6" style={{ borderTop: '3px solid #16a34a' }}>
        ...
        <p className="text-sm text-gray-500 mt-1">Выбери нишу, размер и страну — получишь компании с сайтами. Источник: People Data Labs (CC BY 4.0).</p>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {SIZE_BUCKETS.map((s) => {
                  ...
                  return <button key={s} type="button" onClick={() => toggle(size, setSize, s)} className={`rounded-full px-3 py-1.5 text-sm font-medium border ${on ? 'bg-green-600 text-white border-green-600' : 'bg-white text-gray-600 border-gray-300 hover:border-green-400'}`}>{s}</button>;
                })}
              </div>
            </div>
      ...
      {searched ? (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
              ...
              <table className="min-w-full divide-y divide-gray-200 text-sm">
                <thead className="bg-gray-50">
                  <tr className="text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                    <th className="px-4 py-3">Компания</th><th className="px-4 py-3">Сайт</th><th className="px-4 py-3">Индустрия</th><th className="px-4 py-3">Размер</th><th className="px-4 py-3">Страна</th><th className="px-4 py-3">Описание</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {items.map((r) => (
                    <tr key={r.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3">{r.website ? <a href={`https://${r.website}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-green-700 hover:underline">{r.website} <ExternalLink className="h-3 w-3 text-gray-400" /></a> : <span className="text-gray-400">—</span>}</td>
          ...
          <div className="px-6 py-2 border-t border-gray-100 text-[11px] text-gray-400">Это превью (первые 100). Всю выборку получишь кнопкой «Скачать». Данные: People Data Labs, CC BY 4.0.</div>
        </div>
      ) : null}
    </div>
  );
