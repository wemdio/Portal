...
import type { AdzunaCompanyRow, AdzunaParserJob, AdzunaSearchConfig } from '@/types';
import { AdzunaParserForm } from '@/components/parsers/AdzunaParserForm';
import { AdzunaCompanyResults } from '@/components/parsers/AdzunaCompanyResults';
...
type JobsResponse = { jobs: AdzunaParserJob[] };
type CreateJobResponse = { job: AdzunaParserJob };
type ResultsResponse = { items: AdzunaCompanyRow[]; count: number; limit: number; offset: number };
...
const EXPORT_HEADER = [
  'company', 'domain', 'country', 'roles_found',
  'job_count', 'cities', 'job_titles', 'queries', 'latest_posted_at',
];
...
function exportRow(c: AdzunaCompanyRow): string[] {
  return [
    c.company,
    c.domain ?? '',
    c.country ?? '',
...
export function AdzunaParserView() {
  const [jobs, setJobs] = useState<AdzunaParserJob[]>([]);
  ...
  const [results, setResults] = useState<AdzunaCompanyRow[]>([]);
  ...
  const refreshJobs = useCallback(async () => {
    const data = await apiFetch<JobsResponse>('/api/parsers/adzuna', { method: 'GET' });
    setJobs(data.jobs ?? []);
  ...
  const loadResults = useCallback(async (jobId: string, page: number) => {
    ...
    try {
      ...
      const data = await apiFetch<ResultsResponse>(
        `/api/parsers/adzuna/${jobId}/results?limit=${RESULTS_LIMIT}&offset=${offset}`,
        { method: 'GET' },
      );
  ...
  const fetchAllResults = useCallback(async (jobId: string) => {
    const all: AdzunaCompanyRow[] = [];
    let offset = 0;
    let total = Infinity;
    while (offset < total) {
      const data = await apiFetch<ResultsResponse>(
        `/api/parsers/adzuna/${jobId}/results?limit=${EXPORT_LIMIT}&offset=${offset}`,
        { method: 'GET' },
      );
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`adzuna_parser_jobs_${sessionUserId}`)
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'parser_jobs', filter: `user_id=eq.${sessionUserId}` },
        (payload) => {
          const p = payload as unknown as { eventType?: string; type?: string; new?: AdzunaParserJob; old?: { id?: string } };
          ...
          const next = p.new;
          if (!next?.id || next.parser_type !== 'adzuna_companies') return;
          setJobs((prev) => {
  ...
  // Poll while pending OR running (realtime can be unreliable on self-hosted Supabase).
  useEffect(() => {
    if (!activeJobId) return;
  ...
  const start = useCallback(
    async (config: AdzunaSearchConfig) => {
      setBusy(true);
      setError(null);
      try {
        const created = await apiFetch<CreateJobResponse>('/api/parsers/adzuna', {
          method: 'POST',
  ...
  const exportCsv = useCallback(async () => {
    ...
    try {
      ...
      downloadBlob('﻿' + lines.join('\n'), 'text/csv;charset=utf-8', `adzuna_companies_${activeJobId ?? 'job'}.csv`);
  ...
  const addToDatabase = useCallback(async () => {
    ...
    try {
      ...
      const rows: string[][] = [
        ['Company', 'Site', 'Country', 'Roles', 'Vacancies', 'JobId', 'Parser'],
      ...
      const { id } = writePendingDbImport({ title: `Adzuna #${activeJobId.slice(0, 8)}`, rows });
  ...
  const stopJob = useCallback(async () => {
    if (!activeJobId) return;
    try {
      await apiFetch(`/api/parsers/adzuna/${activeJobId}`, { method: 'PATCH', body: JSON.stringify({ action: 'stop' }) });
      await refreshJobs();
    } catch (e) {
  ...
  const confirmDelete = useCallback(async () => {
    ...
    try {
      await apiFetch(`/api/parsers/adzuna/${deleteCandidate}`, { method: 'DELETE' });
  ...
  return (
    <div className="space-y-6">
      ...
      <AdzunaParserForm onStart={start} busy={busy} />
