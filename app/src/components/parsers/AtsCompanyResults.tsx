...
import type { AtsCompanyRow, ParserJobStatus } from '@/types';
import { Download, Copy, Database, Square, Trash2, Loader2, ExternalLink } from 'lucide-react';
...
export function AtsCompanyResults({
  items,
  count,
  loading,
  jobStatus,
            ...
            <tbody className="divide-y divide-gray-100">
              {items.map((c) => (
                <tr key={c.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-gray-500">{c.ats}</td>
                  <td className="px-4 py-3">
                    <div className="flex flex-wrap gap-1">
                      {(c.roles_found ?? []).map((r) => (
                        <span key={r} className="rounded bg-sky-50 px-1.5 py-0.5 text-xs text-sky-700">
                          {ROLE_LABELS[r] ?? r}
                        </span>
                      ))}
                    </div>
