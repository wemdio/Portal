...
export function Step4Base(props: {
  ...
  // Родитель поллит деталь только при активных джобах, а стадия collect джобы не
  // создаёт — без локального поллинга прогресс-карта сборки замерзала бы. Когда
  // сборка кончается, base_analyze поднимает джобу и родительский поллинг
  // подхватывает analyzing → analyzed.
  useEffect(() => {
