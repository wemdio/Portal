...
/**
 * Read-only «пример готовой цепочки» для демо-портала — вкладка «Цепочки писем»
 * на /client/parsers. Живой инструмент (EmailSequenceV2View) под демо не
 ...
 * сети», третье — прощальное, дописанное в том же тоне. Паттерн компонента —
 * как DemoLaunchPreview: самодостаточная разметка на cp-токенах.
 */
...
const CHAIN: DemoLetter[] = [
  ...
  {
    when: 'через 4 дня',
    subject: 'Закрываю тему — последнее письмо',
    body:
      'Здравствуйте, {{firstName}}!\n\n' +
      'Не хочу быть навязчивым — это последнее письмо. Если пересорт и инвентаризация сейчас не болят, просто проигнорируйте.\n\n' +
      'Если тема живая, но не вовремя — ответьте «позже», вернусь через квартал. А если актуально, 15 минут на демо на этой неделе найдём.\n\n' +
      'Хорошего дня!',
  },
];
    ...
    <div className="neu-sm p-4 sm:p-6">
      ...
      <div className="space-y-5">
        ...
        <section>
          ...
          />
          <p className="text-xs sm:text-sm m-0" style={{ color: 'var(--cp-paper-mute)' }}>
            Розничные сети с 2+ складами (РФ) · ЛПР: директор по логистике, руководитель склада,
            ИТ-директор.
          </p>
