...
Для разработки используй Software Factory из раздела `AGENTS.md` и
[руководства](docs/software-factory.md). Навыки в `.claude/skills/` ссылаются
на общие версии из `.agents/skills/`. Обычная задача заканчивается коммитом,
push своей ветки и отчётом о проверках.
