...
Для задач Portal прочитай [`memory/index.md`](./memory/index.md) и только подходящий вход: Движок вертикалей или датасет Instantly. Опыт и полезные исправления сохраняй по [навыку portal-memory](./.agents/skills/portal-memory/SKILL.md); он также описывает разбор накопленных ошибок. Память не изменяет разрешения из `AGENTS.md`.
